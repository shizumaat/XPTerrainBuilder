...
- 2026-08-11 lane/qtbacklog (Qt backlog QB1-3 + QB4 attribution, spec docs/specs/qt-backlog-parity2-spec.md): 14 Qt test files once, ledgered (200 passed, 28 new in test_qt_backlog_parity.py); import-clean verified. QB1 scan cache at FNAMES.data_path(".tile_scan_cache.json") v1 (atomic write, provider-normalized, key-pair + version guarded); QB2 detector constants 45/30/+5 landed, dash/frozen-clock/tile-clock rules measured ALREADY PRESENT and untouched; QB3 names-only foreign-source audit + screen-point triangle badge (px>14 gate) + info-panel "mixed" row. UNPAID: owner Windows/Linux visual pass (badge legibility across zooms, overlay swap on first rescan); _reaudit_conflict has no UI caller (no texture-cleanup action exists in Qt yet — test-exercised only) [CORRECTED 2026-09-17: STALE. `_reaudit_conflict` now has three UI callers in `src/O4_Qt_GUI.py` (the conflict-resolution paths around lines 1896, 1943 and 2006); this residual is closed]. QB4 STOP-REPORTED, NOTHING BUILT (measured: Qt runs the engine in-process only — no SecretRequest can reach it, jsonl.serve is the only broker installer; parallel-worker secrets serviced parent-side via keyring, Win/Linux backends already bundled; _SignInDialog covers session/http_basic/api_key with registration_url + LoginError surfaces; build-time LoginError WARNING lands in the visible console drawer) — retire-the-item is an OWNER ruling, pending; NEW unfiled gap the measurement surfaced: the macOS Swift app has NO provider sign-in UI at all (Qt is ahead). [CORRECTED 2026-09-17: STALE — closed the same day by lane/swiftsignin, the next entry. The mac app has a Provider Accounts section with a sign-in sheet: `Sources/XPTerrainBuilder/SettingsView.swift` `ProviderSignInSheet` (declared :444, presented :312)]
...
* **THE SPEC'S CAUSE IS REFUTED — deviation RULED at merge** (Fable,
  spec Amendment 1, commit 7f8e1dc: R18-1c ratified).
  ...
  (Ruling delivered: spec Amendment 1, commit 7f8e1dc.)
...
- 2026-08-29 lane/tunneldockets (§T5 wall foot ON at BOTH wall emitters + the last-word foot re-clip; §F1 reaching `_emit_low_corridor_connectors`; the covered-stretch drop naming its coverer): ran the directly-covering files once (test_tunnel_integrity_round, test_lemd_ramp_road_fidelity, test_claimed_corridor_wall_survival) plus `test_no_self_overlap` (SPJC/CYXY/HECA, GREEN with the foot ON). UNPAID: (a) no full suite and no blast-radius suites for bridges.py / pipeline.py / conformance.py, so the last-word re-clip's interaction with the passes AFTER it (final tight T-weld, groundside separation, emit decimation) is proven by the layout-level overlap invariant only, never by an emitted-patch overlap read; (b) the §F1 extension to `_emit_low_corridor_connectors` is proven by SYNTHETIC twins alone — the emitter fires at NEITHER OTHH nor LEMD (0 `tunnel_low_connector` in both patches), so no built patch exercises it and the spec Amendment-1 claim that it owns LEMD's residual twisted bands is unverifiable there; (c) HECA was NOT rebuilt under this tree (three concurrent lanes own it), so the foot's effect on HECA's underpass-cell walls — the +261 class Amendment 2 of the wall-survival spec recorded — has no arm here; (d) `wall_top_flat` CHANGES MEANING with the foot ON (the face legitimately spans foot-top to crest, so the instrument reads the wall's HEIGHT, not a twist) and no instrument change was made — the §F1 law is verified on the foot-OFF arm and by twins, and the ON-arm number is reported as a different measurement; (e) build-time impact unmeasured (the re-clip is one STRtree over the wall pieces; the foot roughly doubles the wall-piece count).
...
- 2026-08-31 lane/ltbatch4a Batch 4a (RULINGS 31e, the ownership shrink at the slice face producer): the batch is REPORTED, NOT MERGED, and its ruled instrument is REFUTED.  (1) `_road_contact_scope` CANNOT express "beyond" over slice faces: its (a) term is `pav_union` grown by 25 m and every slice face is a piece of `pav_union` by construction (`build_global_slice_faces` keeps a face only when `pav.contains(face.representative_point())`), so the ruled rule releases the EMPTY SET — proved by construction, twinned (`test_the_RULED_scope_is_VACUOUS_over_slice_faces_REFUTATION`), and never measured by a build (a build of the default arm is byte-identical by construction and was deliberately not spent).  The same fact explains 31d finding A's measured carve no-op.  (2) The round's experiment knob `O4_SLICE_FACE_OWNERSHIP=movement` (default OFF) was measured on ONE HECA arm `lt4a_heca_arm` (783.6 s, control `lt2c_heca_arm`, no shared-repo writes): 1,307 service-classed faces / 418,118.59 m2 released at the producer and DECLARED in the sidecar; ref-less `service_junction` 1,540 -> 849 rings and 535,859 -> 304,776 m2; the far (>25 m) ref-less subset 1,325 -> 649 rings / 435,882 -> 206,810 m2; `groundside_pavement` 218 -> 898 rings / +267,180 m2; census LAW-TRUE 6,403 -> 4,666 (airside 1,834 -> 1,800, groundside 4,562 -> 2,857).  UNPAID / OWED: (a) the shrink is INCOMPLETE and the named far ring (way -10514 site 30.1142810,31.3772506) is STILL road — attributed by measurement: the movement scope is built from PRE-SCORER face kinds, and scorer v2 later re-classes 1,057 of 2,396 shapes (SERVICE->GROUNDSIDE 557, APRON->GROUNDSIDE 80), so far lot faces carrying no truck route count as "airside" in the region (at the named site the nearest groundside ring is 13.3 m and the nearest emitted airside 48.9 m); the region question — post-scorer class, or the runway touch-chain — is OWED TO THE SPEC AUTHOR and no second region was invented here; (b) the arm is a 29a-class population change and is UNADJUDICATED: airside_value_delta row-side 6,115 nodes moved (worst 6.53 m; 2,349 road-welded / 3,766 soft-receiver), solve-owned 2,449 (worst 6.07 m; 312 / 2,137) — larger than Batch 2's signed-off shift; (c) a NEW airside family fires in the arm (`strip_seam_tear` 0 -> 3 airside, worst 1.58 m) and the step families rise (vertex_to_edge_step 57 -> 71, mid_edge_step 283 -> 354, both groundside-side but for one row) — below the 31f bar, hence STOP-AND-WAIT; (d) no five-airport sweep, no full suite, no app build (per-change files only: test_road_transition 30, test_role_overlap_read 8, test_class_boundary_cut, test_global_slice_hole_keyholes, test_membership_round); (e) the build-time cost of `airside_face_scope` (a unary_union of ~2.4k face polygons + a 25 m buffer) is not isolated — the arm ran 783.6 s against the control's 878.9 s, which is inside the +-25 % single-run band and is NOT a timing claim.

- 2026-08-31 lane/ltbatch4a Batch 4a ROUND 2 (RULINGS 31j, the face-ownership region ruled POST-SCORER): implemented, measured on ONE closing HECA arm `lt4a2_heca_arm` (764.0 s, control `lt2c_heca_arm`, no shared-repo writes), REPORTED AND PARKED — BELOW BAR, not merged.  WHAT THE ARM MEASURES: 1,222 slice-born road shapes / 419,216.36 m2 released and DECLARED (`scope: aircraft_transit_post_scorer`); ref-less `service_junction` 1,540 -> 276 rings and 535,859 -> 118,859 m2; the FAR (>25 m) ref-less subset 1,325 -> 61 rings / 435,882 -> 18,881 m2 (-95.7 %); `groundside_pavement` 218 -> 1,672 rings / +437,599 m2; the minters' populations untouched (`service_road:road` 79 = 79, `service_junction:service` 232 = 232); census LAW-TRUE 6,403 -> 4,289 (adjudicated 4,285; road_cross_section 816 -> 376, transverse 1,738 -> 719, within_shape 2,073 -> 1,485); all three named sites intact (the far ring at 30.1142810,31.3772506 is no longer road; the item-4 contact site and both item-3 missing-road endpoints are bit-identical reads).  UNPAID / OWED, and why it did not merge: (a) TWO NEW AIRSIDE `strip_seam_tear` ROWS (0 -> 2, worst 1.83 m at 61.8 %) — round 1's three rows were pool re-emission re-deriving geometry and ARE fixed (the release now re-roles in place, `simplify_tol=0`), but the residual two are a DIFFERENT mechanism, measured: the gap-fill spine that tears grew from 5,429 m2 (control counterpart) to 100,139 m2, i.e. census #102's blocker interaction — a `service_junction` VETOES a gap-fill hole and a released `groundside_pavement` lot does not, so holes open, spines expand into them and then abut a neighbouring strip with a 1.83 m step.  That is a CONSUMER RULING (does a released lot veto a hole as its road did?), owed to the spec author under the consumer-census law, not a lane guess; (b) `airside_no_step` +41 airside rows (60 NEW / 19 GONE; apron|apron 37, apron|building 16, junction|junction 6; worst new 3.17 m at 8.30 %), unattributed beyond the equilibrium shift; (c) the 29a-class shift is large and UNADJUDICATED: airside_value_delta row-side 7,269 nodes moved, worst 7.90 m (3,558 road-welded / 3,711 soft-receiver), solve-owned 2,200 moved, worst 7.90 m (393 / 1,807); (d) the 61 residual far ref-less rings (18,881 m2, largest 3,316 m2 at 30.1142768,31.3895971) are unattributed — they are not the released population and are likely LATE re-roles (census #74 junction_repair, #101 full-width corridor), which no instrument in this round separates; (e) no five-airport sweep, no full suite, no app build (per-change files only, 195 pass; `test_classification_round::TestLateralContiguityEmitter::test_ring_roads_touching_one_apron_become_one_surface` is a STANDING RED on main, verified in the main tree); (f) the build-time cost of the release pass (a unary_union of the airside shapes + a 25 m buffer + one `_dem_follow_polygon` per released shape) is not isolated — 764.0 s against the control's 878.9 s is inside the +-25 % band and is NOT a timing claim.
...
- 2026-09-04 v2 M4b (lane/v2basins): the mesh apply of the OTHH basin faces (two Dewatering pits, floor + wall band) NOT run (`run_tile_mesh_only.py`; "mesh not required this lane"); CYXY / SPLP / SPJC NOT re-built after the object reader entered the planar stage (a pack with no below-grade geometry is a no-op by construction; one airport per round); the object-read wall (OTHH 11.3 s, LEMD 4.3 s) quoted single-run and concurrent with a second build; the five-airport sweep is the orchestrator's. Owed: a per-resource parse cache for the pipeline stage cache (the OBJ8 parse is ~30 % of the OTHH planar stage).
- 2026-09-04 v2 M4c (lane/v2resid): the plane_gradient INSTRUMENT correction (both readers: plane-fit rounding envelope + undeclared-vertex crown interval) was measured on the two STORED v1 controls (HECA `r1bf_heca_on` 2,811 → 2,809, CYXY `r1bf_cyxy_final` 157 = 157), not on a fresh v1 build; no v1 airport was rebuilt in the lane. SPJC/OTHH/LEMD v2 builds have single-run times only (`--runs 3` on CYXY and SPLP per the brief); no mesh bar run; only the directly covering suites ran once (`tests/auto_patch_v2`, the `tests/test_harness.py` v2/engine selection, the two plane twins) — the full suite was not run. SPLP's v2-verify `within_shape` 1 → 3 (oracle 0) is unattributed.

- 2026-09-04 lane v2rebake (M6a): the v2 re-seat PLAN stage costs 16.7 s at OTHH inside the per-airport auto-patch wall (1,083 members, 147k witnesses; 19 s of it is `obj8.solid_components` over the 1,234 resources the basin pass never decomposed) — over the 0.6 s 1 % line, so the Fable-5 whole-pipeline optimisation review is OWED (candidates: decompose components once for both consumers, cap witnesses before decomposition). The post-mesh seat itself is ~1 s. The closing OTHH tile build ran the re-seat under `O4_DSF_OBJECT_REANCHOR=0` (measured, no pack write): the write path is proven by twins only, pending the owner's ruling on the bridge datum (m6a-report §5).
- 2026-09-04 v2 M1b (lane/v2class, RULINGS 2026-09-04j): no blast-radius runs on the edited files (build economy); the five-airport sweep left to the orchestrator (CYXY closing build + SPLP/SPJC re-census only: oracle 0/0 on all three); the lot TRANSVERSE 2 % is registered in `rulesets.toml` but priced by no generator (a lot has no axis) — owner question in the report; OTHH/LEMD not re-classified this round (lots/strips at a code-4 hub unmeasured).
- 2026-09-05 lane v2integ (M5g, `docs/specs/auto-patch-v2/m5g-report.md`): the seven closing builds ran CONCURRENTLY (seven `build_airport.py --engine v2` at once) — their wall times are correctness-run times, not timing-grade, and the HECA/KCLT IIS-budget burns (+120 s each) were measured only under that load; no `check_build_time.py --runs N`. 04x-2 (the oracle pricing `relaxed_rows` under the "relaxed by 04t(1)" heading) NOT implemented — no airport in the final round is in relaxed mode, so there was nothing to measure it on; the hook is `check_grade`'s `out_of_scope` class mechanism. 04x-1 (QP size gate) skipped (not a cause this round). No blast-radius runs on the edited files; only the directly covering test files (`tests/auto_patch_v2/test_v2integ.py`, `test_relax.py`, `test_constraints.py`, `test_round3.py`, `test_m4.py`, `test_stretches.py`; `tests/test_harness.py` only the new rect-stretch twin) were run once. The five-airport sweep left to the orchestrator.
- 2026-09-05 lane v2padflat (M5h, `docs/specs/auto-patch-v2/m5h-report.md`): the full `tests/test_harness.py` suite was not run (only `-k "v2 or verify or defect"`, 7 green) after adding `verify_defects` to `build_airport.py`'s v2 record; `test_auto_patch_engine_dispatch.py` carries 2 pre-existing reds (`_Res` fixture lacks `rebake_plan`, identical on main 3e95e73f) that the new stage-`verify` failure path in `engine_v2.py` is therefore not twinned against (covered by `test_pad_flat.py` at the reader/DEFECT_KEYS level only); KCLT/HECA not rebuilt under the one-band-per-pad zone law (KCLT stored patch re-read only); no build-time measurement (concurrent correctness builds).
- 2026-09-04 tile-cfg layering (lane, branch claude/intelligent-williams-73e05e): `Tile.read_from_config` now overlays the tile cfg on the global cfg; proven by twins (`tests/test_legacy_cfg_values.py`, 8 layering cases) and the direct-importer suites only (`test_engine_session`, `test_parallel_*`, `test_custom_build_dir`, `test_auto_patch_engine_dispatch`, `test_census_cache`, Qt parity — 220 pass); NO tile rebuilt through the app or `build_airport.py --tile`, full suite not run. `test_auto_patch_engine_dispatch` has 2 pre-existing reds at HEAD 20fde725 (`_Res` stub lacks `rebake_plan` since 5890bfa0) — verified on a clean HEAD control tree, not touched here. The `[provenance] sha=absent version=<O4_Version>` token is twin-proven only; no frozen-engine build log read.
- 2026-09-05 lane v2flatsite (RULINGS 2026-09-05k-2, `lane/v2flatsite`): the closing OTHH build with per-row `flat_datum:<vertex>` groups (c7d89fd5) is NOT at parity with the base arm — 3 v2-verify rows (`tunnel_wall_top_flat` 2, `tunnel_mouth_canonical` 1; oracle census still 0/0) and 265 nodes moved vs dd3ba6b2 (worst 3.71 m, apron ∩ retaining_wall / graded_strip at a sunken-structure rim): the datum preference (5e4) outranks the structure crest/mouth values that are not hard rows. Attempt cap reached; the structure-region exclusion (trim the flat region by the tunnel / basin / wall footprints at its ONE derivation site, `airport/flat_site.region_polygon`) is a spec deviation owed to the spec's Fable author, not decided in the lane. Only `tests/auto_patch_v2` (250) and the ten v1 files touching the changed surfaces (204) ran; no `--runs N` timing; the five-airport sweep is the orchestrator's.
- 2026-09-05 lane v2tunnelobj2 (RULINGS 2026-09-05n, `lane/v2tunnelobj2`): the POST-MESH plate re-seat of the eight OTHH tunnel wall objects (05n-4, `DATUM_PLATE`, delta = ground at the wall band − rendered plate) was not measured on this round's mesh — no tile build (build economy: ONE patch build, `OTHH_20260904T222639`); the seat machinery was replayed with `tools/v2_rebake_replay.py seat` against the M4-era `Data+25+051.mesh` (2026-09-04 04:49, the v2m4 worktree — a mesh that predates every object trench), which exercised the datum on real data (8 plate units, all stations on land; `tunnel1` ×2 HELD there because the OLD mesh differs 0.65 m between its two anchors) but says nothing about this round's deltas; the design-implied deltas are in the sidecar (`reseat_expect_m`) and the per-corridor log line. The five-airport sweep is the orchestrator's (app-build time).

## 2026-09-05 lane v2relaxfull (04t(1) without a certificate, 82e4766f+)
- Oracle census (`oracle.py HECA`, two constant-DEM worlds) NOT run: the closing HECA build went to the ladder (named FAILURE), so the acceptance numbers were taken from the harness census of the real patch (1,604/55); the oracle pair is owed once HECA has a lawful surface.
- `auto_patch/engine_v2.py` per-airport failure on `report.solve.demoted` has no unit twin (the harness build runs `pipeline.build`, not `build_write_verify_one_v2`); covered by the tile-build path at app-build time.
- `tests/auto_patch_v2` was run once serially at 770aac17 (277 passed, the one red = the law/model.py line count, fixed at 82e4766f and re-run for test_model.py + test_law_tables.py only).

## 2026-09-06 lane v2ridge3 (short-pair box as generator law, RULINGS 06s)
- Full engine suite NOT run (pre-ship law): `tests/auto_patch_v2` + `tests/test_harness.py` once serially; the v1 suites whose conftest fixtures transitively import `constraints/taxi.py` (test_pavement_grade etc., blast.py) not re-run — the v1 pipeline never calls the v2 generators.
- The oracle's `_StretchBox.nearest` now falls back to the WHOLE index when the ±2-cell neighbourhood is empty (06s "nearest stretch axis"): the 849 HECA `no_axis` pairs of the 06q reading move into the box population; no separate twin for the fallback beyond `test_v2ridge3`'s `no_axis == 0` assertion.
- CYXY v2-verify carries ONE pre-existing-class residual (airside_no_step §1.2 rate, junction pav15, over by 8e-6 in grade change, below the 0.01 pp floor): the generator's margin equals the worst-case emit rounding exactly and the census frame (equirectangular) differs from the planar CRS by ~0.26 % in scale; not a box row — owed to the no_step generator.
- 2026-09-06 lane v2routecap: the v2 verify's apron-corridor reader (`verify/within.py::taxi_box`, RULINGS 06t) was restricted to gate only its candidate chords through the face cover AFTER the closing HECA build (that build's verify phase read 24.9 s vs 7.1–7.5 s pre-ridge3, ridge3's box reader included); the fix is in the CYXY/OTHH base-arm builds (verify 1.07 s / 115.7 s vs 92.7 s at v2bow3) but its HECA effect is unmeasured — one HECA build per round.
- 2026-09-06 lane v2routecap round 2 (RULINGS 06v): the apron route box now prices EVERY pair of a crossed apron face (HECA pav132's rings run to thousands of vertices); the v2 verify's `apron_pairs` enumeration is O(n²) per crossed face as `within_shape` already was, so no new order of cost, but the box-read wall at HECA is measured only by the ONE closing build of this round — not A/B'd against round 1's 24.9 s verify phase.
- 2026-09-06 lane v2routecap round 2: HECA census 43 (bar 8) — every row an oracle apron chord the generator drops under the 05ae face cover; the oracle's visibility buffer (`GRADE_VISIBILITY_BUFFER_M` 1.0 m, v1 law shared with the v1 solver) differs from the v2 snap margin (0.354 m). Not fixed in this lane (a v1 law constant threaded through `grade_graph.shape_constraints`; which tolerance is the law is a spec question); the v2 verify is the lockstep reader of record until the oracle's buffer is ruled.
- 2026-09-07 lane v2routecap round 3 (RULINGS 06w): HECA v2-verify 32 (bar ≤ 26) — 26 lateral_contiguity pre-existing + 5 strip_seam_tear (two sites on the taxi-E adjacent-ground strips beside pav131, `adjacent_ground:taxi:E:zone1#104/#106` vs `zone2#31/#28`, 1.7 m tears over 3 m; the strips rose 0.4–3.6 m with pav131, which now grades to 1.5 %) + 1 vertex_to_edge_step (apron pav131 vertex 0.8 m from pad building201, 0.63 m). Attributed to the SITE (the tiered apron lifting pav131), mechanism NOT attributed and NOT fixed this round (2-iteration cap; attribution before fix). Census adjudicated 6 (bar ≤ 5): the same 5 + 1.
- 2026-09-07 lane v2routecap round 3: the oracle's `frontage_near_miss` family now reads the tiered cap on a patch declaring `apron_tier` (it read v1's 1 % budget: HECA 2 rows at 1.46 % / 1.57 % that the v2 verify, at the hard cap, does not read). The oracle's apron population also prices every apron ring pair at ANY length (182,985 apron-law pairs vs the generator's 36,844 rows) — its `apron_over_preference` figure (992) is not comparable to the generator's (5,927) or the v2 verify's (2,328); the three readers agree on rows over the HARD cap (0 / 0 / 0 at HECA outside the relaxed set). Pre-existing population divergence (round 2's visibility-buffer note; the nearest-spine-only chord rule), owed to the oracle.
- 2026-09-07 lane v2routecap round 3: 06w (3) — the 5 % back-edge class between adjacent pads (`plan_fan_ramp_zones` port) — NOT implemented; estimate in the spec.

## 2026-09-08 lane v2othhseat (the flat-site datum at the seat, RULINGS 08d)
- Full engine suite NOT run (pre-ship law): `tests/auto_patch_v2` + `tests/test_harness.py` + `tests/test_engine_v2_rebake.py` once serially (726 passed, 1 pre-existing skip).
- `anchor_water_founds_seat = false` (a water anchor takes Z0 on a flat site) is NOT sim-verified: if X-Plane's placement probe returns the water surface (0.00 at OTHH's canal), the 45 Bridges Bus objects render 3.96 m under the banks — v1's owner-accepted OTHH wrote them +1.6 … +4.2 and 1.0.293 +1.7 … +7.8 against the water probe. The owner's sim read of the next OTHH tile decides; the key flips it back.
- Rule (c)'s station read is twin-proven on a synthetic footprint and measured only by the ONE closing OTHH build; the `v2trenchgap` rim position (inside the wall) is covered by the twin, not by a build on that branch.
- The R12 abutment twins (`test_m6b_deck.py`) now run under `anchor_water_founds_seat = true` (a law override in the test): the default law HOLDS a water anchor without a datum, so the landward walk has no default-law fixture.
- 2026-09-08 lane v2othhseat (instance 2): OTHH tile built TWICE (arm 1 inherited, 732 s; arm 2 after the one (e) footprint fix, 745 s) — the second build is the brief's conditional (the fix changed a written family, Terminal 2 → 0); the offline replay (`v2_rebake_replay.py seat` on arm 1's plan + mesh) had already reproduced arm 2's 21 files exactly.
- 2026-09-08 lane v2othhseat (instance 2): `test_constraints.py::test_cyxy_verify_matches_v1_census` failed ONCE in the serial suite while the OTHH tile build and an OTHH census ran concurrently on the machine; passed alone (12.7 s), with its module (12/12) and in a quiet full suite re-run (718 passed, 1 skipped, 137 s). Not attributed (load-sensitive test suspected); the suite's number of record is the quiet run.
- 2026-09-08 lane v2othhseat (instance 2): rule (c) at OTHH tunnels middle-west/-east is REFUTED as the founding mechanism on this tree (spec §6): the objects stand on the corridor ramp, the at-grade ground is nowhere within 8 m of the outer face. The two writes (−2.75 / −2.33) stay; owed to the corridor geometry lanes (v2trenchgap, 08c sunken road).

## 2026-09-09 lane v2seatplanes (rigid-body seats, RULINGS 09b (5))
- The HECA tile build (`build_airport.py HECA --engine v2 --tile 30 31`) was NOT run: the harness REFUSES it because `Elevation_data/+30+030/N30E031_bathymetry_band` is missing from the shared data repo and building would fetch/regenerate it as a side effect (owner ruling e9daef5). The fix is an explicit `--refresh-data dem` — a shared-repo write this lane is not authorised to make. HECA's before/after is measured instead by the offline write-half replay (`tools/v2_rebake_replay.py bodies`) on the owner's own 1.0.296 plan + rebake result: 15,716 stranded components → 2. OWED to the orchestrator: either authorise the `dem` refresh for N30E031 or merge the bathymetry-index fix (memory `session-20260904-bathymetry-index-churn`, branch `claude/laughing-bell-430766`, 0d3da32d, never merged), then rebuild HECA.
- The completion rule's carrier is the NEAREST considered component with no distance ceiling. At HECA a handful of stranded planes sit 140–370 m from their nearest carrier (one .obj covers a whole terminal in the Tai Models pack); they now move with it rather than hang 45 m in the air. Not sim-verified; the owner's read decides whether a distance ceiling is wanted.
- Full engine suite NOT run (pre-ship law): `tests/auto_patch_v2` + `tests/test_harness.py` + `tests/test_engine_v2_rebake.py` once serially (704 passed).

## 2026-09-09 lane v2water (water is a datum, RULINGS 09m (1)(3)(4) / 09o (1)(2)(3))
- The BEFORE mesh is the owner's OWN 1.0.297 tile (`X-Plane 12/Custom Scenery/zOrtho4XP_+25+051/Data+25+051.mesh`, 16:09) — the artifact 09o was attributed on — read with the new `--water-audit`. NOT rebuilt (controls are shared).
- "Does the sawtooth predate the bank?" is answered by EVIDENCE, not by an interventional control: no 1.0.296 mesh exists on this machine and `--base-arm` REFUSES `--tile` (the ledger serves patches, never meshes), so a control would be a second ~12 min tile build on an older tree. The evidence: the 1.0.297 mesh already carried 260 lifted SEA-ONLY vertices (max 5.91 m) and 851 stepped water triangles whose HIGH corner touches no land ring — neither can be a bank ring's doing — and both mechanisms (the flat-site bbox, the INTERP_ALT-seed exemption) are years older than the bank. Not proven interventionally.
- No census / oracle run on the OTHH patch (the round's acceptance is the mesh read); `v2-verify rows 20` is quoted with NO matched control — this worktree's ledger is fresh, and no OTHH base arm exists at this tree.
- The shore itself is UNFIXED by design (09o (4), next round): at the owner's site 189 of 234 water triangles still carry a step, every one of them with its high corner on a vertex shared with a land ring. The bank must daylight at the water's edge before that number moves.
- 14 SEA-only vertices tile-wide still stand off 0.000 (max 3.06 m, was 260 / max 5.91 m). Not attributed — under the materiality floor for this round's target and left to the shore-bank lane's read.
- ROAD BRIDGES OVER WATER, reported not decided: `O4_Vector_Map:88-92` states that a road ribbon over water deliberately keeps its `WATER|INTERP_ALT` (attr 9) triangles so the deck holds its height, and 09o (3) now routes every water-bitted triangle to water levelling. Measured at +25+051: the attr-9 population is IDENTICAL before/after (26,177 triangles, 39.2 km2) and water-only vertices off 0.000 fell 1,107 → 864 with the inland bodies keeping their own levels (37/40/43 m) — nothing sank to zero. Whether a genuine road bridge exists in this tile's attr-9 set was NOT separated; a hillside/estuary airport with mesh road bridges is the read that would settle it.
- Full engine suite RUN once (18 min): 9,947 passed, 13 failed, 24 skipped. The 13 are NOT attributed to this lane and NOT controlled against main (`test_pavement_grade` x5 needs shipped patches; `test_constraints::test_cyxy_verify_matches_v1_census`, `test_contracts`, `test_eat_recognition_scoping`, `test_grade_graph`, `test_strip_rate_reader_frame` x2, `test_tunnel_portal_fidelity`, `test_tunnel_ramp_run_merge`) — none touches water, the mesh classification or the flat-site inset. The lane's own required set is green: `tests/auto_patch_v2` + `test_harness` + `test_patch_seed_seal` + `test_interp_alt_degenerate_face` = 750 passed, 1 skipped.
- 2026-09-09 lane `v2bankblend` round 3 (RULINGS 2026-09-09aa, the vendored Triangle4XP area-test patch): **ONLY `Utils/mac/Triangle4XP` IS REBUILT.** `Utils/src/Triangle4XP.c` carries the restored stock maximum-area test (ahead of the `attribute >= 8` INTERP_ALT exemption) and the mac binary was rebuilt from it on this machine — `cc -O2 -arch arm64 -arch x86_64 -o Utils/mac/Triangle4XP Utils/src/Triangle4XP.c -lm` (Apple clang 21.0.0; universal x86_64+arm64, matching the shipped binary's two slices), sha1 `7ca193114522035fb0449432431b14db02fe37e1`. `Utils/win/Triangle4XP.exe` and `Utils/lin/Triangle4XP` are UNCHANGED and still carry the OLD `testtriangle`: on those platforms `-a` and the FIFTH `.poly` region field (`bank_annulus_region_areas`) remain INERT, the bank annulus gets no interior vertices, and the bank is the ring→foot triangulation §13.7 measured (12.4 % of annulus triangles over the 0.35 bar at HECA). OWED: the release CI (`Utils/CMakeLists.txt`, `Utils/toolchains/mingw-on-linux.cmake` / `osxcross-on-linux.cmake`) rebuilds win + lin from the patched source; until it does, a Windows or Linux user's banks are unblended and the two twins in `tests/test_mesh_bank_annulus_blend.py` (`test_triangle_puts_vertices_inside_a_region_that_asks_for_them`, `test_the_shipped_mac_triangle4xp_is_the_patched_binary`) SKIP there with that reason instead of failing. Also unpaid: no `Utils/CMakeLists.txt` build was exercised (the binary was built with a direct `cc` line, since CMake here sets no optimisation flags at all), and the x86_64 slice was never RUN — only the arm64 slice is measured on this machine.
- 2026-09-09 lane `v2bankblend` round 3, the counterfactual is NOT bit-reproduced: round 2's patched binary (scratch-only, its source never preserved) took the synthetic 30 m annulus fixture to 314 vertices / 130 free in the band; the committed source takes the SAME fixture, same CLI, to 306 / 122. The difference is the `enqueuebadtri` priority argument, which round 2 did not record (stock's `minedge` is not yet computed at the insertion point; the committed patch passes the smallest of the three squared edge lengths). Slope outcome is unchanged — max 0.333 against the 0.333 ring-to-foot slope, 0 % over 0.35 on both arms — so the bar is met either way, but "314" in spec §13.8.4 has no reproduction and the exact Steiner population is toolchain- and priority-dependent.

## lane v2lemdseats round 3 (2026-09-09, RULINGS 09w)

- The LEMD TILE was not built: `build_airport.py LEMD --engine v2 --tile 40 -4` (tag `v2lemdseats_LEMD4`, 470 s) failed at step 2 — Triangle4XP `segmentintersection(): Topological inconsistency`, at LEGT's coordinates in tile +40−004, in geometry this lane cannot touch (LEGT: 0 tunnel objects, 0 basins, 365 stock placements, empty re-seat plan). So the round's |Δ| histogram is measured OFFLINE against round 2's tile mesh from the plan + seat result (`scratchpad/lemdseats/r3/measure6.py`), NOT from a re-baked pack on a fresh mesh, and the ledgered §10.1 numbers (measure5, live pack) are NOT directly comparable — the round-2 column is re-measured with the same instrument instead.
- OTHH's 23 → 25 written is quoted WITHOUT an arm separating this lane from main's `v2water` merge (09u changed OTHH's flat-site datum and water); both arms of this lane read 25, identically.
- `measure6.py` (the seat-residual instrument: authored pack + per-component deltas + one mesh → histogram / worst-N / by-class) is its second use in this lane and is NOT yet promoted to `tools/` with an index entry and a twin, as tool discipline requires. Owed.

- 2026-09-09 (session, whole-suite run on cb577888, before the v2crossing merge 52a73f84): 12 failed / 10,023 passed. Two were the 06u materiality-floor twins in `test_strip_rate_reader_frame.py` (fixed in the same commit as this line). The other ten are STANDING REDS that fail identically at 58cde710 (app 1.0.296 main, measured in a `lane_worktree.sh` control): four v1 source-text assertions (`test_eat_recognition_scoping` probe/restore fence, `test_grade_graph` batched-lambda arity, `test_tunnel_portal_fidelity` clearance annulus, `test_tunnel_ramp_run_merge` chain-emitter text) and the five-airport v1 `test_pavement_grade` battery (within-shape cap 0, superseded by the design-surface acceptance, RULINGS 2026-09-09b). Owed to the ship-gate hardening round; no merge this session introduced a red.

## lane zonemint (2026-09-14, RULINGS 2026-09-13da residual: `zone_on_pavement` minted by the stale sidecar `face_holes`)

- Only the directly-covering test files ran once (`tests/auto_patch_v2/test_v2zonehole.py`, `test_v2hairline.py`, `test_m4.py`, `test_m4b.py`, `test_constraints.py`, `test_pad_route.py`, `test_routes.py`, `tests/test_harness.py`); the full engine suite was not run. The 25 test files importing `pipeline/publication` still call `publication()` and never read `pub["face_holes"]` (grep), so the deleted planar derivation cannot fail them by construction, but that is a static read, not a run.
- `write_tile_pieces` now publishes each tile piece's `face_holes` from the PIECE's faces (a per-piece subset) where it used to write the whole airport's planar holes into every piece; no multi-tile airport was rebuilt to read the census of a seam piece (SPLP / LEMD class). The v1 census reads a piece's faces from that piece, so a face missing from a piece has no way to attach a hole to.

## lane rwyholes (2026-09-14, the RULINGS 2026-09-13dd chips)
...
* the **Qt front end's tests**: `tests/test_qt_boundary_dialog.py` cannot be
  collected in the container (`ImportError: libEGL.so.1`), so the Qt half of
  the boundary gate was not run. No Qt file was edited.
