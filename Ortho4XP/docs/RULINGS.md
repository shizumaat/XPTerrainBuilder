...
## 2026-09-01n — REFUTATION (air4, spawner-ratified): junior-side seat conformance at a solve slot cannot close the tier 1↔2/2↔3 no-step class

* **THE CLASS (01m item 3, attributed in full before the attempt):** 70 imposed-with-no-free-endpoint rows at HECA = 20 tier-1↔2 (junior is the taxiway-family surface; runway may not move) + 50 tier-2↔3 over 16 pads (junior is the seat). Per-pad full senior-partner intersections: 29 rows PARTNERS-CONTRADICT (6 pads — either seniors over-cap between themselves, air2's t2↔2 profile class, or seniors lawful and a FLAT pad cannot span lawful relief: building168, 4.03 m over 397 m = 1.01 %, the split-level-seat trigger); 12 rows OUTSIDE-PAD-BAND (building32 — compliant interval 0.24 m above its own reach-band ceiling); 9 rows in-interval (4 pads). Per-row evidence: the air4 lane scratchpad `air4_stop_evidence.json`.
* **THE MECHANISM TRIED AND DELETED (29f):** a seat clamp in the `pad_seat_consistency` slot — junior-side only, inside the pad's band box, against settled seniors, using the law's own published budgets. Both closing arms (attempt cap 2) regressed the airside census against control 1,073 (@ 000f1ca1, corrected instrument): arm 1 `HECA_20260901T190348` = 1,176 (settled set wrongly included yielding spine values — 4,760 of 4,837 frozen spine nodes yield downstream; a merged unit clamped −3.21 m against a transient value; `apron_lattice_membrane` +103); arm 2 `HECA_20260901T192519` = 1,154 (settled = runway+stations+preserved: `airside_no_step` −18 but lattice +79 / transverse +14, and the no-free-end class itself grew 70 → 74).
* **THE MEASURED LAW:** only 6 of 323 seat↔senior pairs price a value that is CONSTANT at any pre-emit slot — the tier-2 surface does not exist as values until the final projection, so a junior-side clamp at ANY solve slot redistributes the class onto the pairs whose senior lands elsewhere, and the membrane pays for every seat move. Do not re-propose a solve-slot seat clamp against no-step seniors without answering this entry.
* **THE REMAINING CHANNELS (for the owner):** (a) phase-A profile-law ingestion — sound for the 20 tier-1↔2 rows (the runway IS a constant, Amendment 2's free-variable objection does not apply) and for the seniors-over-cap contradictions; routed to the air2 docket (spawner accepted). (b) A post-final-projection tier-3 conform — dissolves Amendment 2's tier-3 constancy; the owner's trade to rule on. (c) Split-level pads (HELD) for the flat-pad-spans-lawful-relief subset — going to the owner with the building168 evidence.
