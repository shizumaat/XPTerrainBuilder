...
the row is stated as the two one-sided `Linear` halves of that equality, HARD
like the pin it replaces: its head `basin.floor = rim - body_depth` is
registered in `[design] hard_rulings`, and `follows` names the FLOOR vertex —
the floor follows, the rim is never pulled.  A basin with no rim vertices or no
...
| `solve/design` §8 | the head is in `hard_rulings`: constraints of the active set, `hard_tol_m` |
| `solve/design` §9 | a vertex a HARD row with `follows` governs ANCHORS its sheet — the floor is no longer "detached" and takes no DEM plane of its own |
