...
### S.8 STEP 6 — AMENDMENT (owner RULINGS 2026-09-18o; Fable 2026-09-18, rev 2) — THE LEVEL-5 LATERAL DETAIL SHRINKS TO THE SAME NEIGHBOURHOOD

APPLY LAST, after rev 1's steps are green. It touches NO rev-1 function
body except the three call-site lines named below. 18o: Q1 (B = 1.0 m) and
Q2 (upstream admission kept) are RULED as rev 1 has them — S.7 Q1/Q2 are
closed, no edit. Q3 reverses rev 1's default (S.4 row 7's last sentence and
S.7 Q3): "Shrink to the patch neighbourhood … elsewhere the tile's normal
road level applies; the road DATA/feed stays."

(1) **ONE NEIGHBOURHOOD, TWO CONSUMERS — ONE NEW SITE.** New
`O4_Vector_Utils.neighbourhood_pieces(linestring, band, runout_m) ->
list[LineString]`: for each connected component of `linestring ∩ band`
take its arclength extent `[s_a, s_b]`, grow it to `[s_a − runout_m,
s_b + runout_m]` clamped to the way, merge overlapping intervals, return
`shapely.ops.substring` pieces (original vertices inside an interval are
kept). Arclength is `way_arclengths`' metric (scalx-scaled degrees ×
`GEO.lat_to_m`) — the clamp's own, never a second one. `band` and
`runout_m` are S.1 (1)/(3)'s, built ONCE in `include_roads` and handed to
both consumers. No reason was found to give lateral levelling a different
neighbourhood: its purpose here is the same join (a level ribbon meeting
the patch's level kerbs), and a second radius would be a second law.

(2) **THE PREDICATE** "this minor-class way gets the level-5 lateral
levelling" = `way ∩ band ≠ ∅` AND upstream's `road_is_too_much_banked`
(unchanged; every such way starts or ends in `apt_array`, so in practice
the first clause decides). In `include_roads`' auto-layer pass ONLY
(`O4_Vector_Map.py:2157-2172`): the `filter` handed to
`OSM_to_MultiLineString` becomes a local wrapper that returns False when
`band` is empty or the way's LineString does not intersect it, else defers
to `road_is_too_much_banked`; then every accepted geom is replaced by its
`neighbourhood_pieces` BEFORE the concatenation at `:2169`. Rev-1 row 5's
`accepted_ids` stays parallel by REPEATING a way's id once per piece. The
two tile-wide passes (`:2104`, `:2131`) are not touched.

(3) **WHAT "THE TILE'S NORMAL ROAD LEVEL" IS UNDER `auto`** (commit
`66a0a67d`, `resolved_road_level` `:692`): `auto → (1, True)` — level 1
tile-wide = `BIG_ROADS_QUERIES` `:46`: `highway=motorway | trunk | primary
| secondary`, `railway=rail | narrow_gauge`, each admitted by upstream's
test (Q2: inside bbox + 1,500 m unconditionally, else the 0.5 m banking
test under `max_levelled_segs`). `road_level ≥ 2` (`small_roads_queries`)
does NOT run under auto, so tertiary / unclassified / residential / service
/ track and siding / spur / yard / tram / light_rail are levelled ONLY as
neighbourhood pieces. A NUMERIC `road_level` keeps its historical tile-wide
classes exactly (that IS that tile's normal level) and the auto-layer adds
only neighbourhood pieces on top. INTERACTION WITH Q2: a minor-class way
inside bbox + 1,500 m but outside the neighbourhood is simply not OFFERED
to upstream's test under auto (it is not in the tile's levelled set), so
upstream's rule admits nothing there; a big-class way there is admitted as
upstream always has. The auto layer's classes never overlap
`BIG_ROADS_QUERIES`, so nothing is double-decided.

(4) **TFFJ POPULATION** (shipped sidecar joined to the road caches; fixture
`population_step6`): 4,429 levelled ways = 283 big-class (131.0 km, 9,093
stations: secondary 179, primary 91, rail 13) + 4,146 minor-class
(564.1 km, 51,501 stations). After step 6: the 283 big-class ways stay
(tile-wide, laterally levelled, terrain longitudinally — 3 of them enter
the band and are clamped per rev 1); of the 4,146 minor-class ways **37
enter the band → 38 pieces, 3.23 km** (service 29, unclassified 5,
residential 3); **4,109 minor ways / ~561 km of ribbon are no longer
built.** Levelled entries 4,429 → ~321. The owner-site way (435,
residential) gets NO ribbon: raw terrain, as the base engine leaves a
residential road at level 1. UNRECONCILED: the findings attribute 4,342
ways to the airport feed; the class join gives 4,146 minor-class — the
difference (196) was not chased; the closing build's log line settles it.

(5) **CENSUS — rows changed / added** (S.4 numbering):

| # | consumer | ruling |
|---|---|---|
| 7 (amended) | `_airport_auto_roads_layer_at` + cache `airport_small_roads` | feed and cache UNCHANGED (18o); only what `include_roads` does with the layer changes — (2) |
| 1 (note) | `clamp_road_network` | no edit. A piece IS its run, so both piece ends are out-of-band run extremities and rev 1 pins them to the DEM — same profile as rev 1 on the whole way up to re-stationing (`refine_way` on the piece). A piece that clips a band corner with no station inside is `scope=terrain`: levelled laterally, not clamped — accepted |
| 8/9 (note) | sidecar + `bank_pavement_lines` `O4_Mesh_Utils.py:1282` | sidecar lists what is levelled → ~321 entries; rev-1 keys unchanged (`layer_way_id` repeats across pieces). A minor road crossing a bank annulus outside any piece now has no ribbon and no pre-valued vertex, so there is nothing for 09ad (a) to protect: the bank field takes it like any ground. Inside a piece: as today |
| 20 | mesh `INTERP_ALT` ribbons (`encode_MultiPolygon :2243`), 13cp Dirichlet `O4_Mesh_Utils.py:2381`, `interp_alt_ribbon_vertex_indices :1413` | fewer ribbons, same rules; no consumer assumes a ribbon EXISTS for a given road (they iterate what is encoded). Expect the mesh's INTERP_ALT node/triangle counts and the harmonic free-set exclusions to fall at every airport with a large inset box — by design, not a regression |
| 21 | `apt_area` difference `:2223` | unchanged; pieces are differenced like any way |
| 22 | v2 `road_coverage_join` / `core_profiles` | unchanged: v2 computes in-process and never read the ribbon. Every way that exits the coverage intersects the band, so it still has a piece (if upstream's test admits it) for the patch kerb to meet |
| 23 | `road_level` hint `O4_Cfg_Vars.py` (+ snapshot) | copy: replace "inside each airport's elevation-inset neighbourhood, where lidar terrain makes unlevelled minor roads most visible" with "where a road meets an auto-patched airport's patch area (about 100 m along the road), so it joins the airport's pavement level" ; regenerate the snapshot in the same pass as row 15 |
| 24 | `tests/test_road_level_auto.py`, `tests/test_airport_road_feed.py` | the feed tests stand; any assertion that every feed way is levelled flips to "only neighbourhood pieces" |

(6) **TESTS** (add to rev 1's files + `tests/test_road_level_auto.py`):
T11 `neighbourhood_pieces`: a 1 km straight way crossing a 50 m band →
one piece of 250 m ± 1 m; two crossings 150 m apart → ONE merged piece; a
way ending 30 m past the band → piece ends at the way end; no intersection
→ `[]`; empty band → `[]`. T12 the wrapper: a minor way inside `apt_array`
but off the band is REJECTED without calling upstream's test; a big-class
way in the tile-wide pass is untouched; ids repeat per piece and stay
parallel. T13 fixture `population_step6` is documentation of the expected
closing-build counts, not an assertion.

(7) **CLOSING-TEST BARS, REVISED** (same ONE TFFJ build): sidecar entries
~321 (300–345; rev 1's "`neighbourhood_ways` ≈ 40" stands); minor-class
entries = pieces only, each ≤ band chord + 200 m; no sidecar way within 8 m
of 17.8985499, −62.838825 and the mesh there = the working raster (10.60 m
± 0.5; shipped 41.16); all rev-1 bars otherwise. BUILD-TIME: expected
LARGE reduction, unmeasured — `improved_buffer` and the INTERP_ALT encode
lose ~561 km of 695 km of linework at TFFJ, and the mesh loses those ribbon
nodes. EMITTABLE: fewer of an existing feature; nothing new.

STEP-6 FILES: `src/O4_Vector_Utils.py` (the new helper only),
`src/O4_Vector_Map.py` (auto-layer pass `:2157-2172` + band hand-off),
`src/O4_Cfg_Vars.py` (`road_level` hint), the schema snapshot
