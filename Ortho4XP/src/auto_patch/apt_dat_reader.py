...
# ``taxi_centerlines`` (the connectivity-routed, bend-split apt.dat taxi
# centrelines) was DELETED 2026-09-17, lane v1retire round 1: its ONLY
# caller was the v1 ``pipeline.py:2737``, and it was the SOLE reason this
# reader — a KEEP module — imported ``pavement.centerlines``, a seam 13aw
# did not know about (it appeared after the inventory).  v2 derives its own
# centre lines in ``auto_patch_v2/constraints/taxi.taxi_centerlines``.
# ``painted_taxi_centerlines`` above stays: it imports no delete module.
