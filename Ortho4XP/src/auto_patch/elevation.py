...
# THE PER-SURFACE SOLVER IS THE SOLVER (user 2026-05-02 / 2026-05-03
# redesign; gate ``O4_PER_SURFACE_SOLVER`` RETIRED 2026-08-05 under
# RULINGS "BUILD-COMPLETE-THEN-DEBUG").
#
# The OFF arm was not "skip the new code": it was a second live
# implementation — the legacy constrained-Laplacian
# ``_solve_pavement_elevations_unified`` plus the legacy terminal
# pre-pin, clamp/subdivide chain and rect-corner snap chain — i.e. a
# whole other world reachable from one env var.  Under "single
# authority everywhere" that world is deleted, not defaulted: the
# ``elevation_per_surface/route_profile`` package carries ALL grade
# law, and there is nothing left for a fallback to mean.
...
def _solve_pavement_elevations(
    ...
    """Run THE elevation solve — ``elevation_per_surface`` — which
    carries every grade law.  There is no second solver to route to.

    A missing DEM is not a fallback condition, it is a LOUD FAILURE:
    the legacy arm that used to absorb it solved a different law, and
    a build that silently grades against no terrain is exactly the
    "silently smaller layout" trap (``standalone-zero-dem-trap``).
    """
    if dem is None:
        raise RuntimeError(
            f"{icao}: _solve_pavement_elevations called with no DEM — "
            "the per-surface solve is the only solver and it needs "
            "terrain.  A caller reaching here has lost its tile DEM; "
            "fix the caller rather than grading against nothing.")
