...
def build_building_seats(layout, bucket_to_idx, band, dem_fn, runway_pts,
    ...
    # THE BINDING ROUTES, published for the sidecar (spec §1.4).  The
    # container is set UNCONDITIONALLY and carries its own node-space
    # stamp: ``nodespace: null`` says the capture could not run, a stamped
    # nodespace with ``records: []`` says it ran and found no pads, and an
    # absent sidecar key says the patch predates the publication.
    setattr(layout, "_pad_binding_routes",
            {"nodespace": _routes_ns, "records": _pad_routes})
    if _pad_routes:
