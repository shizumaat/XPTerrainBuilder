...
def _chord_band(coords, vals, live, a, cap, caps=None, sides=None):
    ...
    ``sides`` — OPTIONAL per-vertex side label for BOUNDARY SEVERANCE
    (spec ``docs/specs/tunnel-corridor-node-book-exclusion-spec.md``
    AMENDMENT 4 §2).  A pair whose endpoints carry DIFFERENT labels
    generates no band at all: inside a tunnel open cut the two sides of
    the boundary are different surfaces — a descending bore floor and the
    at-grade lot beside it — and pricing one against the other over a
    straight chord is precisely the carrier that put the road's bench
    back onto the floor (v2, measured).  Vertices sharing a label keep
    the ring's law among themselves, so both halves stay limited.
    ``None`` = one side for the whole ring, arithmetic bit for bit as
    before."""
    xa, ya = coords[a]
    hi = float("inf")
    lo = float("-inf")
    cap_a = cap if caps is None else caps[a]
    side_a = None if sides is None else sides[a]
    for b in live:
        if b == a:
            continue
        if sides is not None and sides[b] != side_a:
            # SEVERED: this pair straddles the claim boundary.
            continue
        xb, yb = coords[b]
...
def _chord_cut_and_fill(coords, vals, live, free, cap, sweeps: int = 4,
    ...
    the single-cap arithmetic unchanged.  ``sides`` — per-vertex side
    labels for boundary severance (same reference); ``None`` likewise."""
    if not free:
        return
    for _sweep in range(_CHORD_JACOBI_SWEEPS):
        ...
        moves = []
        for a in free:
            lo, hi = _chord_band(coords, vals, live, a, cap, caps, sides)
            v = vals[a]
    ...
    for _sweep in range(max(1, sweeps)):
        changed = False
        for a in free:
            _lo, hi = _chord_band(coords, vals, live, a, cap, caps, sides)
            if hi < vals[a] - 1e-6:
                vals[a] = hi
                changed = True
        if not changed:
...
# to be retried:
#
#   * v1, EXCLUDE THE CLAIM-TOUCHING RING (4e0a7e3c, merged on main at
#     9194b83f and RETIRED by this change): bore floor exact, and the
#     lot half of OTHH ring -12221 rode 1.03 m off the reference with
#     retreat walls 5 of 10;
#   * v2, PER-NODE PRIVATE KEYS ALONE (d83379a8): closed the direct weld
#     channel — the +2.28 bench nodes vanished — but the road's value
#     still won the SAME ring's OUT-OF-CLAIM welds and the ring's own
#     chord law carried it back across the boundary (bore recaptured,
#     walls 2 of 10).  THAT CARRIER IS WHAT HALF 2 SEVERS;
#   * v3, DEMOTING THE ROAD'S PRECEDENCE at claim-touching rings: the
#     bore held, and then the weld — still a SHARED KEY — wrote that
...
def _report_tunnel_corridor_exclusion(layout, stats) -> None:
    ...
    """
    by_role = stats.get("tunnel_corridor_rings_touched") or {}
...
def _node_inside_tunnel_claim(x, y, prepared, bounds) -> bool:
    ...
    which SIDE of the boundary it sits on for severance (half 2).  A
    second predicate would be a second geometric notion of "inside the
    cut", which the spec's one-authority clause forbids.
    """
    ...
    * a node carried by two roles takes the STRICTER of their caps, and
    ...
    for i, s in enumerate(layout.shapes):
        ...
        if _claim_prep is not None:
            ...
            # ── HALF 2: BOUNDARY SEVERANCE ───────────────────────────
            # Only a ring carrying BOTH sides has a straddling pair to
            # withhold; a ring wholly in or wholly out keeps the scalar
            # path, arithmetic bit for bit as before.
            if _n_priv and _n_priv != len(sides):
                ring_sides[i] = sides
                stats["tunnel_corridor_severed_rings"] += 1
        else:
            keys = xy_keys
        rings[i] = keys
        ring_xy[i] = xy_keys
    ...
    for _round in range(4):
        ...
        for i, keys in rings.items():
            ...
            # arithmetic is bit for bit as before.
            coords = ring_xy[i]
            # HALF 2: the side labels, present only for a ring that
            # carries both sides of the claim boundary.
            sides = ring_sides.get(i)
            ...
            if any(c != cap for c in caps) or sides is not None:
                # A node this ring shares with a STRICTER role: the pair
                # budget drops to the stricter cap at that endpoint.  A
                # severed ring takes the same path for its side labels.
                _chord_cut_and_fill(coords, vals, live, free, cap,
                                    caps=caps, sides=sides)
            else:
