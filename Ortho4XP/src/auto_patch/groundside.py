...
# The ring-axis reader, the ≥45 ° classifier, the cap resolver and the
# road role set, IMPORTED from the law.  ``grade_graph.shape_constraints``
# classifies the SOLVE's road pairs with these same four; this module's
# late chord limiter is the one law RUN LATE (spec
# ``docs/specs/road-surface-quality-spec.md`` §2.2), so it must reach the
# same objects — a second copy of a 45 ° test here would be two laws over
# two populations.
from .grade_law import (
    ROAD_ROLES as _LAW_ROAD_ROLES,
    long_axis_of_points as _long_axis_of_points,
    pair_is_transverse as _pair_is_transverse,
    road_cross_section_cap as _road_cross_section_cap,
)
from .config import ROAD_CROSS_SECTION_LAW as _ROAD_XSECTION_LAW
...
from .grade_law import (                   # noqa: E402
    ROAD_ROLES as _LATERAL_ROAD_ROLES_LOCAL,
    ring_path_cumulative as _GL_RING_PATH_CUM,
    road_pair_distance as _GL_ROAD_PAIR_DISTANCE,
)
...
def _chord_band(coords, vals, live, a, cap, caps=None, axis=None, pad=None,
    ...
    """``(lo, hi, lo_pin)`` — the ``cap``-reachable band vertex ``a`` may
    sit in, generated by every other live vertex of the ring.

    ``lo_pin`` is the same floor restricted to the PINNED generators (the
    welds; ``-inf`` when ``pinned`` is None or generates none).  It is
    what lets the kernel apply RULING 1 — THE WELD OUTRANKS THE CAP —
    when the band comes out EMPTY; see :func:`_chord_cut_and_fill`.
    ...
    ``axis`` — OPTIONAL ``(ux, uy)`` ROAD AXIS (owner ruling RULINGS
    2026-08-25g, "ROADS ARE LATERALLY FLAT").  Given one, a pair whose
    chord runs ACROSS it is the road's CROSS-SECTION and its budget is
    ``grade_law.road_cross_section_cap`` of the cap it would otherwise
    have had — the SAME two law functions ``grade_graph.
    shape_constraints`` classifies the solve's pairs with, so this late
    pass and the solve cannot disagree about which pairs are lateral.

    WHY THE LATE PASS NEEDS IT, measured (this lane, CYXY, the seam
    ledger): this limiter's band is ISOTROPIC at the role cap, so it
    permits — and re-creates — a lateral tilt anywhere under 8 %.  With
    the solve enforcing the cross-section, the limiter's two runs were
    the two seams that MINTED lateral defects back: +8 lateral pairs
    over the 2 % limit at ``14_groundside_separation`` and +10 at
    ``20_post_projection_conformance``, on a final population of 171.
    A pass that moves a road node with no law pricing that direction is
    the defect (spec ``road-surface-quality-spec.md`` §2.2), and this is
    the remedy the spec names: the same law objects, run late.

    ``None`` (every non-road ring, and the whole pass with
    ``O4_ROAD_CROSS_SECTION_LAW=0``) is byte-identical arithmetic.

    ``pad`` — OPTIONAL per-vertex BUILDING-PAD claim flags.  BUILDINGS
    ARE THE HEAVIEST CONSTRAINT (user 2026-07-03; owner ruling in the
    25g round): a pair with a pad-claimed endpoint is a FRONTAGE CHORD,
    not a road cross-section, and the 2 % band may never pull a road
    node off its pad.  ``grade_law.classify_pair`` already reaches this
    verdict on the solve side — its building clamp runs BEFORE the
    cross-section branch, which is ``min``-shaped and therefore the
    identity on an already-1 % frontage pair — and this is the same
    verdict in the LATE pass.  RELAXING ONLY relative to the
    cross-section: the pair keeps the LONGITUDINAL cap it had before
    25g, never anything looser."""
    xa, ya = coords[a]
    hi = float("inf")
    lo = float("-inf")
    ...
    for b in live:
        ...
        if dn > lo:
            lo = dn
        if pinned is not None and b in pinned and dn > lo_pin:
            lo_pin = dn
    return lo, hi, lo_pin
...
def _chord_cut_and_fill(coords, vals, live, free, cap, sweeps: int = 4,
    ...
    ``axis`` — the ring's ROAD AXIS, so its CROSS-SECTION pairs clamp at
    ...
    build, must not contradict it.  So: an empty band whose FLOOR is
    generated by a PINNED vertex resolves UP to that floor, and the
    guarantee sweep never cuts below it.  A vertex whose infeasibility
    comes from free neighbours only keeps the cut-only disposition
    exactly (``lo_pin`` is ``-inf``), and in a FEASIBLE band
    ``lo_pin <= lo <= hi`` makes every clause below the identity — the
    arithmetic is unchanged everywhere except at a weld the cap cannot
    reach."""
    if not free:
        return
    pinned = set(live) - set(free)
    for _sweep in range(_CHORD_JACOBI_SWEEPS):
        worst = 0.0
        moves = []
        for a in free:
            lo, hi, lo_pin = _chord_band(coords, vals, live, a, cap, caps,
                                         axis, pad, path, pinned)
            v = vals[a]
            if lo > hi:
                target = lo_pin if lo_pin > hi else hi
            else:
    ...
    # but never below a floor a WELD generates (ruling 1 above).
    for _sweep in range(max(1, sweeps)):
        changed = False
        for a in free:
            _lo, hi, lo_pin = _chord_band(coords, vals, live, a, cap, caps,
                                          axis, pad, path, pinned)
            if lo_pin > hi:
                hi = lo_pin
            if hi < vals[a] - 1e-6:
...
def _airside_claimed_keys(layout):
    ...
    return _claimed_keys(
        layout, lambda r: r is not None and r not in GROUNDSIDE_ROLES)


#: The BUILDING-PAD roles, for :func:`_pad_claimed_keys` — both spellings
#: the layout mints for a pad, because the frontage law is about the PAD,
#: not about which producer made it.
...
def _claimed_keys(layout, want):
    """``(xy_keys, canonical_keys)`` for every node claimed by a ring
    whose role satisfies ``want`` — the ONE claim walk behind both
    :func:`_airside_claimed_keys` and :func:`_pad_claimed_keys`.

    Factored so the two pin sets cannot drift into two different ideas of
    what "a ring claims this node" means.
    """
    keys: set = set()
    canon: set = set()
...
def _grade_limit_groundside_chords(layout) -> int:
    ...
    * a node carried by two roles takes the STRICTER of their caps, and
    ...
    ring_role: dict = {}
    ring_cap: dict = {}
    #: ring index -> its ``(ux, uy)`` ROAD AXIS (RULINGS 2026-08-25g);
    #: absent for every non-road ring and with the law gated off.
    ring_axis: dict = {}
    #: ring index -> per-vertex BUILDING-PAD claim flags (only for rings
    #: that have at least one).  A pair touching one is a frontage chord,
    #: never a cross-section.
    ring_pad: dict = {}
    stats: dict = {
        "rings": {}, "changed": {}, "caps": {}, "nodes": 0,
        "road_xsection_rings": 0,
        "road_xsection_frontage_rings": 0,
        "road_xsection_frontage_nodes": 0,
    ...
    for i, s in enumerate(layout.shapes):
        ...
        # THIS RING'S OWN AXIS, once, from THE law's reader — the same
        # ``grade_law.long_axis_of_points`` ``grade_graph.
        # shape_constraints`` classifies the solve's road pairs with, and
        # the same one the lateral-contiguity station walk reads a road's
        # direction with.  Computed over the ROUNDED ring keys, which is
        # exactly the coordinate list the kernel below clamps over, so
        # the axis and the pairs it classifies come from one geometry.
        #
        # ONE KILL SWITCH, ONE MEANING: ``O4_ROAD_CROSS_SECTION_LAW=0``
        # gates the census, the solve AND this late pass together —
        # they are one law and they land together (spec §1.3).
        if _ROAD_XSECTION_LAW and role in _LAW_ROAD_ROLES:
            _rax = _long_axis_of_points(keys)
            if _rax is not None:
                ring_axis[i] = _rax[0]
                stats["road_xsection_rings"] += 1
                # THE FRONTAGE EXEMPTION, per ring vertex.  Same claim
                # notion as the airside pin and resolved in the same two
                ...
                if any(_pf):
                    ...
                    stats["road_xsection_frontage_nodes"] += sum(_pf)
    ...
    for _round in range(4):
        ...
        for i, keys in rings.items():
            ...
            # other ring — which is what keeps their arithmetic bit for
            # bit what it was.
            axis = ring_axis.get(i)
            pad = ring_pad.get(i)
            # Amendment 1 clause 1: a ROAD ring's pairs are priced along
            # its own walk, through THE law function the census uses.
            path = None
            if (_ROAD_PATH_METRIC
                    and ring_role.get(i) in _LATERAL_ROAD_ROLES_LOCAL):
                path = _GL_RING_PATH_CUM(keys)
