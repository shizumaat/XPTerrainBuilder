...
def _bake_basin_rim_flush_facilities(
    ...
    *,
    ...
    from .object_terrain_kinds import BASIN_RIM_FLUSH_DECISION_KIND
...
def _bake_basin_group_seat_facilities(
    ...
    *,
    ...
    from .object_terrain_kinds import BASIN_GROUP_SEAT_DECISION_KIND
...
def _abutment_line_frame_samples(start, end, offset=(0.0, 0.0)) -> list:
    ...
    from .object_terrain_kinds import _ABUTMENT_GRADE_SAMPLE_STEP_M
...
def _mint_seat_coalition_finding(
    ...
    from .object_terrain_kinds import BRIDGE_SEAT_COALITION_FINDING
...
# ``_report_bridge_findings`` and ``rebake_dsf_objects`` — THE RETIRED v1
# PHASE 2 — MOVED to ``post_mesh_v1.py`` (seam S3, lane v1retire
# 2026-09-17).  Stage A (RULINGS 2026-09-13az) stopped calling them:
# ``O4_Mesh_Utils`` dispatches ``engine_v2.rebake_after_mesh`` only.  They
# were the sole reason this KEEP module called into
# ``object_terrain_assembly`` (``post_mesh_object_terrain_records``), so
# they move to the v1 side and are deleted with it in round 2.
