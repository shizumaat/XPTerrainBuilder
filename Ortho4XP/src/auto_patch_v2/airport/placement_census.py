...
# ── §17: THE COCKPIT FRAME, object stage ─────────────────────────────────
# (owner RULINGS 2026-09-12x/12y; object-placement-spec.md §17;
#  design-surface-spec.md §31 (6))
#
# A CLASSIFICATION of the bars above, never a new reading: every number
# here comes out of a census dict this module already produced, and the
# block asserts that what it classified adds up to what it was handed.
# The two thresholds and the approach range are the SAME law keys the
# terrain census reads (``emit.toml [cockpit]`` through
# ``auto_patch_v2.law.tables.cockpit``) — one frame for both stages.
