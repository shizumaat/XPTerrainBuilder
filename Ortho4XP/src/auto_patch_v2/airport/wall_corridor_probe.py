...
# ── THE TERMINAL WITNESS (RULINGS 2026-09-10ac-1 (A) / 10ao) ─────────────

@_dc.dataclass(frozen=True)
class TerminalWitness:
    """The terminal nearest a corridor's mouth and how far away it is."""

    distance_m: float
    witness: str


def terminal_polygons(airport: Airport) -> list[tuple[str, Polygon]]:
    """Every TERMINAL the airport states: the ``aeroway=terminal`` ways of
    the tile's ``airports`` feed (an Overpass export flattens a
    multipolygon relation into its member ways, so a relation's outline
    arrives here as a way).  X-Plane's ``apt.dat`` carries NO terminal
    geometry — its rows are pavement, lights and startup locations — so
    OSM is the only source; that is a fact about the format, not a gap in
    this reader.  Frame coordinates."""
    out: list[tuple[str, Polygon]] = []
    for w in airport.osm_ways:
        if w.tags.get("aeroway") != "terminal" or len(w.points) < 3:
            continue
        p = Polygon(w.points)
        if not p.is_valid:
            p = p.buffer(0)
        if p.is_empty or p.geom_type != "Polygon":
            continue
        out.append((w.tags.get("name") or f"osm way {w.id}", p))
    return out


def terminal_witness(terminals: _t.Sequence[tuple[str, Polygon]],
                     mouths: _t.Sequence[XY]) -> TerminalWitness:
    """The nearest terminal to any of a corridor's mouths.  ``inf`` with
    the reason when the airport maps no terminal at all."""
    if not terminals:
        return TerminalWitness(math.inf, "no aeroway=terminal in the airports feed")
    if not mouths:
        return TerminalWitness(math.inf, "no mouth")
    best = min(((float(p.distance(Point(m))), nm) for nm, p in terminals for m in mouths),
               key=lambda t: t[0])
    return TerminalWitness(best[0], f"{best[1]} at {best[0]:.1f} m")
