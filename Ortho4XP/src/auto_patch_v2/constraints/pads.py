...
def _pad_rows(planar: PlanarMap, law: Law, cap: float, ruling: str,
    ...
    # PLANE within its 1 % ceiling, full stop: the vertices it shares
    # with airside are vertices of the plate like any other (09-01g,
    # contact = value), priced two-sided at the cap this call was asked
    # for.  The whole band machinery — the rigid core, the one-way and
    # then two-sided skirt row at a relaxed ceiling, the both-ends-
    # airside withdrawal, ``[placement] pad_skirt_m`` and ``[within_
    # shape] pad_skirt_max_slope`` — is DELETED with it.
    #
    # WHY, MEASURED (this lane, rounds 2-3).  The skirt existed because a
    # rigid plane cannot meet a CURVED apron edge (14ay): the difference
    # had to go somewhere, and 14al/14au put it in a 5 % skirt band.
    # 14ay's answer is that the APRON must be planar where it meets the
    # pad — §30 (4)'s collar — and with a collar there is nothing for a
    # skirt to absorb.  Round 2 measured the cost of keeping both: with
