...
from ..model.rebake import (DATUM_CLUSTER, DATUM_DECK_TOP, DATUM_PLATE, PLAN_FILENAME,
                            PLAN_VERSION, ClusterSeat, Member, MemberSeat, PadRequest, Part,
                            RebakePlan, SeatResult, Unit, UnitSeat)
...
__all__ = ["seat", "deck_datum_from_surface", "Sampler", "Part", "Member", "Unit",
           "RebakePlan", "MemberSeat", "UnitSeat", "ClusterSeat", "PadRequest", "SeatResult",
...
def _structure_seat(u: Unit, base: float, sampler: Sampler, rb, br
    ...
    coalition and the clearance confirmation (module doc)."""
...
def seat(plan_: RebakePlan, sampler: Sampler, law: Law) -> SeatResult:
    ...
    for ui, u in enumerate(plan_.units):
        ...
        if a is None:
            for mi in range(len(u.members)):
                base_by[(ui, mi)] = None
            continue
        rec["anchor_ground"] = float(a[0])
        base = rec["base"] = float(a[0]) + u.agl_m        # the rendered y = 0 plane
        for mi in range(len(u.members)):
            base_by[(ui, mi)] = base
        datum, delta, founding, readings, findings = _structure_seat(u, base, sampler, rb, br)
    ...
    out = _cl.seat_clusters(plan_, sampler, law, base_by, fixed, family)
    units: list[UnitSeat] = []
    for ui, u in enumerate(plan_.units):
        rec = per_unit[ui]
        ...
        if rec["anchor_ground"] is None:
            units.append(UnitSeat(u.id, resources, None, DATUM_CLUSTER, None, None, tuple(
                MemberSeat(m.resource, DATUM_CLUSTER, None, 0, 0, 0, 0, "anchor off the mesh")
                for m in u.members), "held: anchor off the mesh", (), True))
            continue
        base = rec["base"]
