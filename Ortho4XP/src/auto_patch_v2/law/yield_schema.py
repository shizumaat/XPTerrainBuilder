...
YIELD_CLASSES: tuple[str, ...] = ("taxi", "apron", "road", "runway_contact")
...
class Yield:
    ...
    #: THE RUNWAY CONTACT (owner RULINGS 2026-09-08r-1): the taxi chain
    #: where it meets a runway-family vertex conforms to the runway with NO
    #: ceiling (v1's junctions reach 9 %) — this class never states one;
    #: ``check_yield`` refuses a value.
    runway_contact_yield_max: float | None = None
...
def check_yield(y: Yield, err: type[Exception]) -> None:
    ...
    if y.runway_contact_yield_max is not None:
        raise err("emit.yield.runway_contact_yield_max: the runway contact has NO ceiling "
                  "(owner RULINGS 2026-09-08r-1) — delete the key")
