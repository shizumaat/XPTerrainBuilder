...
__all__ = ["Sparse", "to_sparse", "Problem", "assemble"]
...
def to_sparse(cs: ConstraintSet, n: int) -> Sparse:
    """See ``ConstraintSet.to_sparse``'s contract."""
    eq_r: list[int] = []
    eq_c: list[int] = []
    eq_v: list[float] = []
    eq_b: list[float] = []
    ...
    for ln in cs.linears:
        ...
        if ln.hi is not None:
            ub(ln.terms, ln.hi, ln)
        if ln.lo is not None:
            ub(tuple((v, -c) for v, c in ln.terms), -ln.lo, ln)
    for b in cs.bands:
        if b.lo is not None:
    ...
    return Sparse(A_eq, np.asarray(eq_b, float), A_ub, np.asarray(ub_b, float),
                  lo, hi, tuple(eq_rows), tuple(ub_rows))
...
def assemble(planar: PlanarMap, cs: ConstraintSet, weights: Weights) -> Problem:
    ...
    A_ub_cs = sp.hstack([S.A_ub, sp.csr_matrix((S.A_ub.shape[0], n_t + n_r))],
                        format="csr")
    A_eq = sp.hstack([S.A_eq, sp.csr_matrix((S.A_eq.shape[0], n_t + n_r))],
                     format="csr")
    ...
    ub_rows = tuple(S.ub_rows) + (None,) * row
