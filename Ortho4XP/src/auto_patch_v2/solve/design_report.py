"""THE DESIGN SOLVE'S REPORT — :class:`DesignReport` and :func:`residual`.

Split out of ``solve/design.py`` under the 1,000-line file law (``tests/
auto_patch_v2/test_model.py::test_no_v1_import_no_env_gate_no_geometry_in_
model``), exactly as ``airport/obj8_clip.py`` was split out of ``obj8.py``:
the design solve's OUTPUT VOCABULARY — the per-family / per-term residual
record the pipeline and the sidecar read, and the row-kind certificate —
is a coherent block that reads the solve's law registers not at all.
``solve/design`` re-exports both names, so every importer and twin that
reads them from there keeps working; this module is where they live.

NO BEHAVIOUR CHANGE: the two definitions moved verbatim.
"""
