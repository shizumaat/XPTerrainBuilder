...
class TestTheTrenchIsOpenOnlyWhereNothingCoversIt:
    """§33 (6) B AMENDED (2) (RULINGS 2026-09-15bp): the emitted cut is
    the trench MINUS the object's own cover plate ∪ the live airside
    pavement, pads and unit footprints over it.  Over the covered stretch
    the SURFACE holds its own law — no rim, no wall, no floor ring, no
    `tunnel_ramp` face, so `taxi_centreline` and every airside row see
    surface vertices only (r1 measured the opposite: TUNNEL2_DONE's floor
    ring carried a taxi-centreline vertex and pulled the junction beside
    it down 6.46 m)."""

    @staticmethod
    def _shell():
        from shapely.geometry import Polygon
        trench = Polygon([(0, 0), (300, 0), (300, 20), (0, 20)])
        foot = Polygon([(-2, -2), (302, -2), (302, 22), (-2, 22)])
        return trench, foot

    def test_the_cover_splits_the_cut_into_its_open_parts(self, law):
        from shapely.geometry import Polygon
        from auto_patch_v2.planar import structure_geometry as sg
        trench, foot = self._shell()
        # a taxiway lying ACROSS the middle 100 m of the shell
        cover = Polygon([(100, -10), (200, -10), (200, 30), (100, 30)])
        g = sg.geometry_from_trench(lambda s: (s, 10.0), [0.0, 150.0, 300.0], 8.0, 0.7, 0.5,
                                    trench, foot, covered=cover)
        assert g is not None
        assert g.ramp.geom_type == "MultiPolygon" and len(g.ramp.geoms) == 2
        assert g.outer.geom_type == "MultiPolygon" and len(g.outer.geoms) == 2
        # NOTHING is emitted under the cover — not a floor ring, not a rim
        assert g.ramp.intersection(cover).area == pytest.approx(0.0, abs=1e-6)
        assert g.outer.intersection(cover).area == pytest.approx(0.0, abs=1e-6)
        # …and each open part keeps its wall all round it: the band at the
        # open↔covered transition IS that transition's portal face
        assert g.ramp.distance(g.outer.boundary) >= 1.2 - 1e-6

    def test_a_fully_covered_shell_emits_nothing(self, law):
        from shapely.geometry import Polygon
        from auto_patch_v2.planar import structure_geometry as sg
        trench, foot = self._shell()
        cover = Polygon([(-50, -50), (350, -50), (350, 70), (-50, 70)])
        assert sg.geometry_from_trench(lambda s: (s, 10.0), [0.0, 150.0, 300.0], 8.0, 0.7, 0.5,
                                       trench, foot, covered=cover) is None

    def test_a_cover_that_touches_nothing_leaves_the_cut_whole(self, law):
        from shapely.geometry import Polygon
        from auto_patch_v2.planar import structure_geometry as sg
        trench, foot = self._shell()
        far = Polygon([(1000, 1000), (1100, 1000), (1100, 1100), (1000, 1100)])
        whole = sg.geometry_from_trench(lambda s: (s, 10.0), [0.0, 150.0, 300.0], 8.0, 0.7, 0.5,
                                        trench, foot)
        got = sg.geometry_from_trench(lambda s: (s, 10.0), [0.0, 150.0, 300.0], 8.0, 0.7, 0.5,
                                      trench, foot, covered=far)
        assert got is not None
        assert got.ramp.area == pytest.approx(whole.ramp.area, abs=1e-6)

    def test_a_sliver_the_cover_leaves_is_dropped_not_emitted(self, law):
        """An open part too small to carry its own wall band on both sides
        is a crumb, not a trench: emitting it would put a floor ring back
        within a grid step of the surface."""
        from shapely.geometry import Polygon
        from auto_patch_v2.planar import structure_geometry as sg
        trench, foot = self._shell()
        # the cover leaves 0.5 m at the east end and 100 m at the west
        cover = Polygon([(100, -10), (301.5, -10), (301.5, 30), (100, 30)])
        g = sg.geometry_from_trench(lambda s: (s, 10.0), [0.0, 150.0, 300.0], 8.0, 0.7, 0.5,
                                    trench, foot, covered=cover)
        assert g is not None and g.outer.geom_type == "Polygon"

    def test_the_rim_path_carries_every_open_parts_ring(self, law):
        from shapely.geometry import Polygon
        from auto_patch_v2.planar import structure_geometry as sg
        trench, foot = self._shell()
        cover = Polygon([(100, -10), (200, -10), (200, 30), (100, 30)])
        g = sg.geometry_from_trench(lambda s: (s, 10.0), [0.0, 150.0, 300.0], 8.0, 0.7, 0.5,
                                    trench, foot, covered=cover)
        want = sum(len(p.exterior.coords) for p in g.outer.geoms)
        assert len(g.left_rim) == want and g.right_rim == []


class TestCoverRegion:
    """`structure_service.cover_region` — the two halves of the cover, one
    union, and only for a signature-B corridor."""

    @staticmethod
    def _corridor(cover=None):
        ...
        class C:
            ...
            trench: Polygon
            cover: object = None
        box = Polygon([(0, 0), (300, 0), (300, 20), (0, 20)])
        return C("object-cut:t.obj@0", box, box, cover)
    ...
    def _cells(role):
        ...
        class Cell:
            ...
            role: str
            kind: str = "pavement"
        polys = [Polygon([(100, -10), (200, -10), (200, 30), (100, 30)])]
        return [Cell("pav1", role)], polys, STRtree(polys)

    def test_live_airside_pavement_over_the_trench_is_the_cover(self):
        from auto_patch_v2.planar import structure_service as ss
        cells, polys, tree = self._cells("junction")
        got = ss.cover_region(self._corridor(), cells, polys, tree, [], None, ("junction",))
        assert got is not None and got.area == pytest.approx(polys[0].area)

    def test_a_role_outside_the_protected_set_is_not_a_cover(self):
        from auto_patch_v2.planar import structure_service as ss
        cells, polys, tree = self._cells("service_road")
        assert ss.cover_region(self._corridor(), cells, polys, tree, [], None,
                               ("junction",)) is None

    def test_the_objects_own_cover_plate_is_the_other_half(self):
        from shapely.geometry import Polygon
        from auto_patch_v2.planar import structure_service as ss
        plate = Polygon([(0, 0), (50, 0), (50, 20), (0, 20)])
        got = ss.cover_region(self._corridor(cover=plate), [], [], None, [], None, ())
        assert got is not None and got.area == pytest.approx(plate.area)

    def test_only_a_signature_B_corridor_has_a_cover_region(self):
        from auto_patch_v2.planar import structure_service as ss
        c = self._corridor()
        c.id = "tunnel-object:wall.obj@0"
        cells, polys, tree = self._cells("junction")
        assert ss.cover_region(c, cells, polys, tree, [], None, ("junction",)) is None
