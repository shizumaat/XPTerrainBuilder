...
def test_seat_senior_constraints_are_stashed_for_the_clamp():
    """Airside-zero round (RULINGS 2026-09-01m item 3): every IMPOSED
    cross-tier pair whose junior end is a TIER-3 seat and whose senior
    end is tier 1/2 is stashed on ``layout._nostep_seat_cons`` —
    ``{seat_idx: [(senior_idx, budget)]}`` in THIS solve's node space —
    for ``pad_seat_consistency.apply_seat_no_step_clamp`` one statement
    later.  Tier2<->tier2 and membrane pairs never enter it."""
    ap = _Shape(_rect(-60.0, -20.0, 0.0, 20.0), "apron")
    jn = _Shape(_rect(0.0, -20.0, 20.0, 20.0), "junction")
    layout = _Layout([ap, jn])
    bucket_to_idx, node_pos, ctx = _wire(layout)
    n = len(node_pos)
    ids = {(round(px, 3), round(py, 3)): i for i, (px, py) in node_pos.items()}
    jn_nodes = {ids[(round(x, 3), round(y, 3))]
                for (x, y) in list(jn.polygon.exterior.coords)[:-1]}
    ap_only = {ids[(round(x, 3), round(y, 3))]
               for (x, y) in list(ap.polygon.exterior.coords)[:-1]} - jn_nodes
    tiers = ANS.tier_of_nodes(n, centerline_nodes=jn_nodes,
                              seat_nodes=ap_only | jn_nodes)
    sc, senior, recs, rep = ANS.build_airside_no_step_constraints(
        layout, bucket_to_idx, ctx, node_pos=node_pos, n_nodes=n,
        tier_of=tiers)
    cons = layout._nostep_seat_cons
    assert cons, "no seat constraint was stashed"
    # every entry: junior is a SEAT (tier 3), senior is tier 2 here
    for j, lst in cons.items():
        assert tiers[j] == ANS.TIER_SEAT
        for (s, bud) in lst:
            assert tiers[s] == ANS.TIER_CENTERLINE
            assert bud > 0.0
    # the stash is exactly the imposed cross-tier seat population
    n_pairs = sum(len(v) for v in cons.values())
    n_expected = sum(1 for r in recs
                     if r["imposed"] and {r["tier_a"], r["tier_b"]} ==
                     {ANS.TIER_SEAT, ANS.TIER_CENTERLINE})
    assert n_pairs == n_expected > 0
