...
def test_the_calibrated_PASS(oracle, cg, monkeypatch, tmp_path):
    ...
    (10.0, 13.0) everywhere.
      ...
      * band_width: every node 13.0 − 10.0 = 3.0; nodes 5, pinned 0,
        negative 0, and none over the ruled pair's seed span
        (10 000 − −500 = 10 500 m, reported beside the counts so the
        envelope the numbers were judged in is in the record) ⇒ PASS.
    ...
    assert v["band_width"]["summary"] == {
        "nodes": 5, "pinned": 0, "negative": 0, "over_span": 0,
        "seed_span_m": 10500.0, "envelope_m": [0.0, 10500.0],
        "min": 3.0, "p50": 3.0, "max": 3.0, "mean": 3.0}
