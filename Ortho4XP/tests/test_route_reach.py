...
The fix landed in two parts (2026-06-28): a reach band for the
no-centerline feeders — since 2026-07-29 the single band engine's own
off-route LEG covers them (the separate ``_build_skeleton_band``
fallback, which had no service filter, was deleted) — and the
...
def test_cyxy_route_reach_zero():
    ...
    feasible for all its feeders.  The band's off-route leg reaches the
    no-centerline west apron over its own pavement; the no-building apron seat
