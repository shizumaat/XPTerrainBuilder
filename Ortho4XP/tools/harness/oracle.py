...
def main(argv=None) -> int:
    ...
    if lo != hi:
        field = band_width_field(layouts[lo]["_layout"],
                                 layouts[hi]["_layout"])
        keys = field
        summary = band_width_summary(field, span_m=seed_span_m(lo, hi))
        ...
        verdicts["band_width"] = {
            ...
            "note": "a NEGATIVE band width is a defect on its face "
                    ...
                    "a width OVER THE SEED SPAN (constant_dem.seed_span_m "
                    "of the pair built) means something amplified the "
                    "seed — the envelope is DERIVED, never assumed",
        }
