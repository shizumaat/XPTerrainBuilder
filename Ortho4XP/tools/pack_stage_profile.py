...
def run_once(icao: str, cache_on: bool, out_dir: Path) -> dict:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if a.one:
        print("RECORD " + json.dumps(run_once(a.icao, a.cache == "on", a.out_dir)), flush=True)
        return 0
    runs: list[dict] = []
    for k in range(a.runs):
        cmd = [sys.executable, __file__, a.icao, "--one", "--cache", a.cache,
               "--out-dir", str(a.out_dir)]
        env = dict(os.environ)
        if a.tree:
