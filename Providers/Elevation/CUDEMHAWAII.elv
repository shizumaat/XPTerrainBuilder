# NOAA NCEI Continuously Updated DEM (CUDEM), ninth arc-second Hawaii
...
# /search API -- discovery walks the catalog. Unlike New Zealand's tree, these
# catalogs link their tiles directly from the root (rel="item", no child
# collections); the static_stac strategy synthesizes one pseudo-collection
# whose bounding box is coverage_bbox below.
access_strategy=static_stac

# Root STAC catalog for the NCEI ninth arc-second topobathy Hawaii dataset.
...
# pseudo-collection's bounding box. Covers the main Hawaiian islands.
coverage_bbox=-160.5,18.5,-154.5,22.5

# Vertical datum. CUDEM Hawaii is referenced to a LOCAL TIDAL datum
