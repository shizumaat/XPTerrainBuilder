...
| SPJC-3 | OPEN | SPJC | Tunnel mouths gone at -12.0326702, -77.115129 and -12.0325443, -77.1148715 (were emitted) — regression | ATTRIBUTION ONLY, no interventional measurement (`docs/findings/beta2-CYXY-SPJC-regressions.md`): v1 patch has tunnel_ramp+tunnel_wall at both mouths; suspect §34 (12) (5) `terrain_tunnel_witness` (`eab1a904`) + Law C sibling refusal (both mouths die together). Needs a v2 SPJC build; lane `tunnelwitness` (#5): measured NOTHING covers bore -5724 (nearest pack OBJ 63.2 m, pack polygons within 40 m = shrub .for covering 0 m, DEM falls 21.97 -> 21.44); no witness invented — OWNER Q-5 on the issue. |
| HECA-1 | DIAGNOSED | HECA | Nested building shapes: two building20's, one inside the other; only the larger footprint matters | #6 — attributed (lane `hecabodies`): the concrete slab's fallback pad overlapped the T3_20 cluster pad by 2,281 m² via a lettering band's convex-hull ring; branch `claude/hecabodies` 5a9c3eb6/40e81a5a/ee2a4386 removes every overlapping pad pair (21 → 0); residual at the site: the cluster lobe still sits in a weld-closed hole of the slab pad (issue comment) |
| HECA-2 | DIAGNOSED | HECA | building13 ends at 30.112611, 31.4058943; the part south/east at 30.1125298, 31.4064909 is not it and must be free to terrace | #7 — the retaining wall's footprint ring inflated 113 → 7,385 m²; branch `claude/hecabodies` 40e81a5a + d96e733f: the point is on no pad in the closing build `hecabodies_close5` |
| HECA-3 | DIAGNOSED | HECA | Service road should extend to 30.1122535, 31.4062746; object wall on the road edge is a RETAINING wall — road level at wall top, ground beyond at wall bottom | #8 — owner question Q-8 on the issue (law not in §47); the false building13 pad over the road end is removed by the #7 fix |
| HECA-4 | DIAGNOSED | HECA | Terminal floating roof elements + roof-mounted light posts at 30.1110619, 31.4041921 | #9 — the §16g unit-seat carrier exemption was disarmed by db414ec1 (`if False:  # ARM-OFF`); re-armed on branch `claude/hecabodies` 72a24ff1: riders on their own ground within 60 m 160 → 2 |
| HECA-5 | DIAGNOSED | HECA | Many buildings / larger terminals missing interiors or windows, or levels separated | #10 lane `hecainteriors` (S6 contents, #30): §16g (10) (4) left every LEAF inside a building (glass, doors, slabs, frames) seated on its own ground — on main 6fca486d 3,499 of 11,874 contained leaves stand apart from the walled body over them (2,768 > 0.5 m; `black_glass_NOANPHA` 773). Branch claude/hecainteriors: contents join their host unit (one zero) + a file never straddles two units -> 1,150 (966 > 0.5 m); T3 building4 district 81 -> 1; patch body byte-identical (4a6d82bdc0df). Residual: connector-host contents (fu:43:5027, 1.88 m), the Private_hall chain (#66). |
| HECA-6 | DIAGNOSED | HECA | Cargo complex 30.1194398, 31.4077874 → 30.1161247, 31.4087786 must not be sunken; weld to the apron (30.1172704, 31.4094737) along the whole east edge | #11 — pads and the east apron never touch (20–40 m DEM gap); §20 levels the pad to the NE apron at 93.3 (DEM under it 96–99); owner question Q-11 on the issue |
| OTHH-1 | OPEN | OTHH | No tunnel exists at 25.2575296, 51.6120308; extra tunnels emitted around the groundside in front of the terminal (25.2584256, 51.6142851) || lane `tunnelwitness` (#12): Law C kerb test (d) — a wall pair whose OWN wall rises > 3.0 m over the object's zero is foundations; OTHH wall-corridor records 73 -> 11; site a 0 structures within 60 m (was a ramp at 0 m); site b: VCN_004 corridors gone, OSM bore `-10442` (into parking pad building2) remains 15.3 m away. OWNER Q-12 on the issue. |
...
| OTHH-5 | OPEN | OTHH | Tunnel wall 25.2639569, 51.6126961 broken: should be a straight rectangle matching the pack's object wall, at TERRAIN height not below grade; ramp a simple sloping rectangle, not wiggling || lane `tunnelwitness` (#16): walled cap corner is the mitre; `Terminal_Base_2_1@4` rim 2.61–3.40 -> 3.94 (ground 3.96), ramp top 3.37 -> 3.94, the pad infeasible set gone (emit replay). |
