...
| NLWF-3 | OPEN | NLWF | Tiny apron and buildings too elevated to be reachable from the runway | #20. Attributed (lane nlwf): the pack's apron is a HARD solid Y = 0 OBJ (`pavement/vele_apron.obj`, 1,499 m², no draped layer group) that §42 refused, so the ground under it followed the DEM 5.7 → 12.8 m. §42 (1b) hard ground plane admitted (branch claude/nlwf): replay apron 5.3–7.6 m, terminal pad 15.9 → 7.5 m, gate ALL ZERO; owner Q-20 (ratify). Awaiting sim read. |
