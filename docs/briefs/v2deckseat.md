# Brief pack — lane `v2deckseat`

Base: main `511d2718` · generated 2026-09-17 by `tools/brief_pack.py`
...
Lane v2deckseat: §49 a parapet rides the deck — deck face datum, shear to the deck's grade, low-end fallback

## The brief

# Lane `v2deckseat` — spec §49: a parapet rides the deck (owner RULINGS 2026-09-17u-2 / 17x (2); Fable §49; scout `deckseat`)

Read §49 in your pack IN FULL — it is the spec, clause by clause, with the bars in (11). Read `docs/briefs/deckseat-report.md` for the consumer census (30 passes, one table) and the 22-body population table; every file:line you need is there (base 390c3282 — re-locate after merging main: lane `v2leafseat` edits `anchor_rule.py`, `placement_plan.py`, `footprint_unit.py`, `placement_family.py` and MERGES BEFORE YOU START; merge main first, then re-cite).

Order of work: (1) twins first (§49 (11): the synthetic 4 % deck + 60 m wall sheared within 1 cm at both ends; the ANIM wall at the low end, 0 float; the 1/81 wall untouched; the carried body rides its carrier; a BASIN body untouched) in `tests/auto_patch_v2/test_v2deckseat.py` (new, the class is new) plus the existing `test_v2objsplit.py` unchanged; (2) the region reader + `refs_many` (§49 (1)); (3) `deck_datum_of` (3) and the three mints (4) — smallest diff at each, `placement_plan.py` is over the 1,000-line law so move `_own_ground_file` out to `placement_ground.py` (or similar) first; (4) the shear at `_retok` (5) with normals, and the refusals; (5) the fallback (6); (6) the cut (7) — measure Bridge2's body count before/after; (7) the dry LEMD pair through `tools/obj8_split_report.py` (both arms, base via `git archive`), the §49 census line, then KCLT / HECA / OTHH / VHHH / LGAV byte-identity on their shipped products; (8) ONE LEMD harness build; (9) §49 MEASURED block + RULINGS entry (next free key, re-grep the tail in the same command).
Peer lanes live: `v2channelfp` / `v2f6mouth` (planar/), `v2neckarm` (classify/), `v2interiors` (§48: `airport/interiors.py`, `load.py`, `obj8` reads) — never touch their files. Never write the shared data repo. The only LEMD mesh on disk is 2026-08-27 and UNMATCHED: dry-to-dry only.

## Bars

1. Twins green (§49 (11)); `test_v2objsplit.py` unchanged.
2. Dry LEMD pair: `Bridge2` b0/b2/b3/b4, `LEMD50` b0, `LEMDzaun` b34 within `visual_m` 0.5 of `bridge_deck:-6288` at every station after the shear (today +4.54 / −0.23 / −6.87 / −6.88 / +0.03 / −2.00); b1 (pier) unchanged; Bridge2 one body per wall or the abutment cut named.
3. `-11828`: nine carried bodies unchanged, `LEMD60__b8` unchanged, `LEMD60` b11/b12, `LEMD38` b76, `VOR-40-T1` b0 before/after named.
4. `Bridge4.obj` crest-plate datum restored through §10 (delta named).
5. KCLT byte-identical (3 decks, 0 bodies); HECA / OTHH / VHHH / LGAV byte-identical; `unit_leaf_bodies`, `bodies_bound_to_unit` unchanged.
6. §49 census line green or every residual named; counters published; every fallback / refused body named.
7. ONE LEMD closing build rc 0, harness census with the cockpit block, `[harness] shared repo UNCHANGED`.
8. Suite green (quote FAILED lines); products under `/tmp/harness/v2deckseat/` registered; §49 MEASURED + RULINGS on the branch; report branch + sha + items NOT done.

## Files

Yours: `Ortho4XP/src/auto_patch_v2/airport/placement_read.py`, `Ortho4XP/src/auto_patch_v2/airport/anchor_rule.py`, `Ortho4XP/src/auto_patch_v2/airport/placement_cut.py`, `Ortho4XP/src/auto_patch_v2/airport/placement_body.py`, `Ortho4XP/src/auto_patch_v2/airport/placement_plan.py`, `Ortho4XP/src/auto_patch_v2/airport/placement_boxes.py`, `Ortho4XP/src/auto_patch_v2/airport/placement_carrier.py`, `Ortho4XP/src/auto_patch_v2/airport/obj8_split.py`, `Ortho4XP/src/auto_patch_v2/airport/placement_census.py`, `Ortho4XP/src/auto_patch_v2/law/structures.toml`
Other lanes' (do not touch): `Ortho4XP/src/auto_patch_v2/planar/`, `Ortho4XP/src/auto_patch_v2/classify/`, `Ortho4XP/src/auto_patch_v2/airport/interiors.py`, `Ortho4XP/src/auto_patch_v2/airport/load.py`, `Ortho4XP/src/auto_patch/`

## Spec (design-surface) §49
...
## 2026-09-17y scout `deckseat` REPORTED (report `docs/briefs/deckseat-report.md`) and SPEC §49 WRITTEN (511d2718): the deck region is 14 `bridge_deck:*` faces corpus-wide, all `service_road` groundside, 22 bodies on one at LEMD and 0 anywhere else; DECK ∩ PAD = ∅, DECK ∩ RIM = 8–9 nodes; the surface already carries the deck's z — the defect is `ground_under`'s MEDIAN over boxes half on the trench; three mints pre-empt any datum in `anchor_for` (§10's station cut already discards `Bridge4.obj`'s live crest-plate datum); the seat is a pure translation at `obj8_split._retok` so a SHEAR is one site (normals too; an `ANIM` body cannot be sheared → the owner's low-end fallback); 100 m stations on a 4.12 % deck step 4.1 m against `visual_m` 0.5 — the shear, not finer stations

* Consumer census (30 passes, one table, in the report): pads arm INERT; rims arm IGNORE (a deck shares rim nodes; fed as a rim it would class parapets BASIN); the surface/ground arm is where the wrong z comes from (`placement_boxes.ground_under :194-202`, `_LineCutter.terrain_groups` cuts a deck-straddling wall in two BEFORE any seat — Bridge2 component 1 → b3 + b4); pre-emptions `placement_body.py:143-163` → `segment_anchor`, `placement_carrier.is_elevated :73-106`, `placement_plan._own_ground_file :299-330`. Exclusions by construction: BASIN bodies (`LEMD60__b8`, `Bridge3__b0`), CARRIED bodies (9 on `-11828`), datum-bearing bodies, deck-class members, unit/family members; the pier `Bridge2` b1 is out by the on-fraction test (1/81). 13r QUOTED: it bound the draped deck PLATE ("no object re-seat"), never the parapets (measured b3/b4 −6.9 m, b0 +4.5 m — nothing 13r describes).
* §49 (design-surface spec, after §48; object-spec §16e AMENDED pointer): (1) ONE region reader `placement_read.decks_from_graded_doc` → `DeckFace(ref, ring, z)`, `DECK_REF_PREFIX = "bridge_deck:"`, role-agnostic, `GradedRoles.refs_many`; (2) population = ≥ `[deck] on_fraction` 0.5 of feet/box samples inside one deck, exclusions above; (3) `anchor_rule.deck_datum_of` = the deck's own surface at each point; (4) the three mints consult it first, `is_elevated`'s datum escape holds, carriers read `ground_off` against the deck; (5) the body is SHEARED to the deck's grade between its end stations at `_retok` with normals transformed, refused by name for `ANIM` / `|g| > shear_max_grade 0.10` / `shear = false`; (6) fallback = the low end, never a float; (7) the terrain cut reads the deck-substituted ground for a body on a deck; (8) 13r stands for the plate; (9) a §49 cockpit line at `visual_m`; (10) keys `[deck] on_fraction / shear / shear_max_grade`; (11) bars on the dry LEMD pair (Bridge2 b0/b2/b3/b4, LEMD50 b0, LEMDzaun b34 within 0.5 m of `-6288` at every station; b1 unchanged; KCLT/HECA/OTHH/VHHH/LGAV byte-identical; `Bridge4` plate restored; suite; ONE LEMD build).
* Lane `v2deckseat` (brief `docs/briefs/v2deckseat.md`) is PACKED and WAITS for `v2leafseat` to merge — both edit `anchor_rule.py` / `placement_plan.py`. Instrument note: the scout's region sweep is a first-use scratchpad script; on second use extend `obj8_split_report.py --rows-on-ref PREFIX`, never fork. `tools/INDEX.md` exists at the repo root (the lemdobjects "mismatch" was a dirty-tree artefact).
