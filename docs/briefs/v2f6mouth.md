...
Lane v2f6mouth: a ramp primarily off pavement stops at the pavement edge (§34 (12) (3) AMENDED, owner 17x (4)); the strip covers ground not a neighbour's pavement (§34 (5) (b))

## The brief

# Lane `v2f6mouth` — LEMD F-6 underpass: the north mouth ramp must stop short of the pavement band `pav188` (owner RULINGS 2026-09-17q item 2, 17v, 17w (3))

Read `docs/briefs/lemdramp-report.md` — the scout's full attribution is the spec of the defect. Summary:
- Way −10863 is the north mouth ramp of OSM bore `tunnel:-5821+-5820@1` under `aeroway=taxiway bridge=yes layer=1` way −1230 (F-6). The mouth 40.4613118 = centreline + (0.15 m kerb offset + 19.0 m code-E strip − 2.1 m erosion): §34 (5) (b) adds the deck cell `pav157`'s graded strip on a cell whose north kerb is 0.15 m from the centreline, so the strip lands on the NEIGHBOUR's pavement `pav188` (14.5 m inside it; the rim notches 324 m² out of it). §34 (12) (3) frees `pav188` through the portal's-own-pavement exemption (`structure_service.py:283-284`), and the truncation loop only drops the LAST station (the top). §34 (8)/(9) do not apply (wall corridors only).
- Owner's end point: 40.4613855,−3.5447716 (8.331 m north of the current mouth). A pure truncation keeping the record's climb (564.9 → 571.62 over 84 m) needs 8.88 % > the 8 % cap → the lawful form MOVES the mouth and lengthens the covered trench by 8.33 m. Reported, unexplained: the emitted north ramp runs 571.52 → 570.0 (−1.79 %), 6.62 m above the record's `mouth_z` 564.9, while the south ramp honours 564.9 — attribute it (`v2_solve_replay --why-vertex` / a structures pair) and say whether the north ramp is a shallow cut by design (14bl: "the DEM is already low there").
- History: 13ai mouth 40.4612284; 14bt moved the north extent back 7.5 m; 15h (§34 (5) (b)) pushed it +9.3 m north into `pav188`. The cure for 15e item 7 (the 5.38 m unbanked trench face inside zone 1 of `pav157`) must be KEPT: `pav157` never cut, no trench face opening inside its zone 1.
- Peer lane `v2neckarm` is re-roling `pav188`'s band to taxiway concurrently (17w (3)); your fix must hold on BOTH roles (the scout measured that the role does not govern this mouth; prove it on your pair).

## Order of work
1. Twins (`tests/auto_patch_v2/test_v2lemdstruct.py` / `test_v2rampwalk.py`, extend): a deck cell whose kerb is < 1 m from the centreline with a wide neighbour pavement on that side → the strip stops at the neighbour's boundary (b1); a mouth standing 14 m inside a neighbour cell → the exemption covers only the rim footprint + stand-off and the ramp is stopped short (b2); the 14bt/15h fixtures unchanged.
2. Implement b1 at `structure_underpass.strip_half_width_m` / `_deck_cell` (`:58-82`, `:472-473`): the strip covers GROUND — clip each station's offset at the first airside cell boundary beyond the deck cell's own kerb. b2 at `structure_service.py:283-284`: exempt only the part of the portal's cell within the rim's footprint + `wall_gap_m + wall_band_width_m`.
3. Dry pair `python -m auto_patch_v2.planar LEMD --stage structures` (base via `git archive`, ~190 s/arm; durable base arm `/tmp/harness/v2wallface/base_LEMD/structures.json` at 31b7ad1b is NOT your base — cut your own at your base sha) → `tunnels[]` `tunnel:-5821+-5820@0/@1` `mouth_ll`/`rim_ll`, `underpasses[]` ribbon area (2,186 m² / 50 stations today). Controls: KCLT taxiway U (−1560) mouths byte-identical; VMMC; LEMD's other 46 tunnels unchanged or named.
4. ONE closing harness build: LEMD (`build_airport.py LEMD`), census, `osm_site.py --relate` for the `pav188` notch, `airside_value_delta` vs the 1.0.348 shipped patch (`/Users/noah/XPTerrainBuilderData/Patches/+40-010/+40-004/LEMD_auto.patch.osm`).
5. Spec §34 (5) (b) / (12) (3) AMENDED + MEASURED; RULINGS entry (next free 17x key, re-grep in the same command).

## ADDENDUM — owner RULINGS 2026-09-17x (4), THE RAMP-MOUTH LAW (verbatim): "there are times a tunnel ramp is inside airside or groundside pavement, however any time a tunnel ramp is primarily off pavement and leading down toward pavement, it should almost certainly stop at the pavement edge."
Implement b2 as THIS law, not as a rim-footprint carve-out: decide per corridor whether the ramp is PRIMARILY OFF pavement (name the measure — the fraction of the ramp ring's area or length outside every pavement cell; say what threshold you chose and why); if it is, and it leads toward a pavement cell, the mouth (the ramp's high end) lands ON that pavement's edge (the portal's-own-pavement exemption does not apply); a ramp primarily INSIDE pavement keeps today's behaviour. The F-6 north ramp (14.5 m inside `pav188`, ~70 m outside) is the primarily-off case. Twin both cases.

## Bars

1. Twins green; existing underpass/rampwalk twins unchanged.
2. North mouth within 1 m of 40.4613855,−3.5447716; the covered trench longer by ~8.3 m; ramp monotone, ≤ 8 %; the south mouth unchanged at 40.460868.
3. `pav188` notch 324 m² → 0 (or ≤ 160 m² named); `pav157` never cut; no trench face inside `pav157`'s zone 1 (the 15h bar holds).
4. KCLT taxiway U mouths byte-identical; LEMD's other tunnels unchanged or every difference named; the north-ramp z asymmetry attributed.
5. ONE LEMD closing build rc 0, census with the cockpit block, `[harness] shared repo UNCHANGED`, airside deltas named.
6. Suite green (quote FAILED lines); products under `/tmp/harness/v2f6mouth/` registered; spec MEASURED + RULINGS on the branch; report branch + sha + items NOT done.

## Files

Yours: `Ortho4XP/src/auto_patch_v2/planar/structure_underpass.py`, `Ortho4XP/src/auto_patch_v2/planar/structure_service.py`, `Ortho4XP/src/auto_patch_v2/planar/structures.py`, `Ortho4XP/src/auto_patch_v2/law/structures.toml`, `Ortho4XP/tests/auto_patch_v2/test_v2lemdstruct.py`, `Ortho4XP/tests/auto_patch_v2/test_v2rampwalk.py`
Other lanes' (do not touch): `Ortho4XP/src/auto_patch_v2/planar/channel.py`, `Ortho4XP/src/auto_patch_v2/planar/channel_floor.py`, `Ortho4XP/src/auto_patch_v2/planar/channel_geometry.py`, `Ortho4XP/src/auto_patch_v2/classify/`, `Ortho4XP/src/auto_patch_v2/airport/`, `Ortho4XP/src/auto_patch/`
