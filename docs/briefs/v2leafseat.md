...
Base: main `be0bc21a` · generated 2026-09-17 by `tools/brief_pack.py`
...
Lane v2leafseat: a leaf body seats in the unit its elevated members belong to (§16g (10) (4) AMENDED); one pad datum, the median; pad faces folded
...
## ADDENDUM — scout `lemdobjects` (RULINGS 2026-09-17u, report `docs/briefs/lemdobjects-report.md`): LEMD T4 is the OTHER half of the same law
- At LEMD the unit `fu:25:983@cluster_pad` DID form (45 members + 19 carried = 64 bodies) and seats ALL of them at 614.77 = `min(p.z)` over the WHOLE ring of ONE face of pad `building45` (five faces share the ref; `pad_plurality` at `placement_family.py:543` / `anchor_rule.py:266` stores the LAST `PadRing` matched — iteration-order datum, 0.41 m swing). The pad's low vertex is 590 m from the owner's points; the apron `pav12` under them is 615.16–616.62; the reasons print "own ground +0.42…+2.24 m" (median +1.26). Meanwhile `Terminal4_05` b5 and `_56` b0 stayed OUTSIDE the unit at 616.96 — 2.37 m inside one building.
- IN SCOPE NOW (ruled 17t + pure defects): the MEDIAN datum for both seats; fold EVERY face of a pad ref in `pad_plurality`/`pad_majority` before min/median (fix C, a pure defect — touch `anchor_rule.py` LAST, 21 importers); fix A for HECA T3 (the leaf's elevated members). Measure the T4 sites on the LEMD replay before/after and REPORT the residual: with median, expect T4 still ~0.8–1.4 m under `pav12` at the owner's points — that residual is owner question 17u-1 (local datum by reach vs the airside surface as a floor under every member) and is NOT yours to fix until the session forwards the ruling; do not implement fix B.
- NOT yours: Bridge2 parapets (17u-2, a new deck region → spec + consumer census first).
- LEMD replay: prefer `tools/obj8_split_report.py` on the shipped `o4_v2_rebake_LEMD.json` + `LEMD.graded.json` (no mesh); the only LEMD mesh on disk (`Ortho4XP/Tiles/zOrtho4XP_+40-004/Data+40-004.mesh`, 2026-08-27) is UNMATCHED to the 1.0.348 vector run — say so if you use it.
...
7. `pad_plurality` / `pad_majority` fold every face of a ref (twin: a five-face pad whose faces' minima differ → one datum whatever the iteration order).
8. LEMD replay: T4 sites' per-body zero vs graded `pav12` before/after (median datum); `Terminal4_05` b5 / `_56` b0 inside-vs-outside the unit named; the residual under 17u-1 reported, not fixed.

## Files

Yours: `Ortho4XP/src/auto_patch_v2/airport/footprint_unit.py`, `Ortho4XP/src/auto_patch_v2/airport/placement_plan.py`, `Ortho4XP/src/auto_patch_v2/airport/placement_family.py`, `Ortho4XP/src/auto_patch_v2/airport/anchor_rule.py`, `Ortho4XP/src/auto_patch_v2/law/structures.toml`, `Ortho4XP/tests/auto_patch_v2/test_v2leafframe.py`, `Ortho4XP/tests/auto_patch_v2/test_v2objsplit.py`, `Ortho4XP/tests/auto_patch_v2/test_v2connector.py`
