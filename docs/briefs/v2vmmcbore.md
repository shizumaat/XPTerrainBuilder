# Brief pack — lane `v2vmmcbore`

Base: main `e9364382` · generated 2026-09-16 by `tools/brief_pack.py`
...
No terrain tunnel for a bore that enters a building (§34 (12) (5)) — VMMC 0 tunnels; LEMD/OTHH unchanged

## The brief

Owner 2026-09-16 (RULINGS 16d): no tunnels at VMMC — all roads above ground; bridges/overpasses/ramps are the sim's elevated roads. The surviving bores after v2vmmcshore r6 are car-park ramps under Taipa's buildings (`tunnel:-2488@0`, `-2088` on `-4787@1`). Sites: `planar/structure_approach.py` (`mouths()` — the admission gate after 12ab), `planar/structure_service.py` (r2 added `airside_stops`/`osm_stops`; add the building/parking witness here), `airport/deck_signature.py` (`is_tunnel_way`), the OSM feed loader (`airport/osm.py`, `load.py` — which layer carries buildings/parking), `law/structures.toml`. Other lanes running: v2shellwall r3 (planar/structure_geometry.py, structures.py, constraints/taxi.py, structure_underpass.py — do NOT touch structures.py; if the admission needs a structures.py line, say so and stop), v2padclip, v2shoulderband r2, the peer's v2channel (planar/channel*.py, planar/basins.py, structures.py). Never write the shared data repo or the X-Plane install; never --refresh-data; no --tile.

## Bars

- Consumer census (08-30l) of the bore-admission readers (`planar/structure_approach.mouths`, `deck_signature.is_tunnel_way`, `structure_service`, the OSM feed readers for `building=*` / `amenity=parking` / `parking=*` / `covered=*`) in the MEASURED block BEFORE the edit; name the feed(s) that carry the building/parking witness for each tile (if none does, say so — the rule then needs a feed change, report and stop).
- VMMC matched replay pair (the registered VMMC capture; re-cut on the merged tree): tunnels 13 → 0, ramps → 0, rims → 0, `-2488@0` and `-2088`/`-4787@1` named with their building/parking witness; the owner's probe 22.1618794,113.579745 covered by nothing; the sea wall / quays / `sea_wall` 27 rows / pav5 5.09…6.12 unchanged from r6's build.
- LEMD dry pair: tunnels 55/56 unchanged tunnel for tunnel (real road tunnels, layer −1/−2 under the field); OTHH 44 unchanged (pack corridors are exempt); SPJC / KCLT / CYXY dry pairs: any bore that changes named with its witness (a real terrain tunnel must survive).
- Twins: a bore ending in a building footprint (not built), in an underground parking (not built), a bore under pavement with a mouth on the field (built), a pack-stated corridor (untouched).
- Suite by FAILED lines; ONE VMMC airport-path build (~20 s) as the closing test; `shared repo UNCHANGED`.

## Files

Yours: `Ortho4XP/src/auto_patch_v2/planar/structure_approach.py`, `Ortho4XP/src/auto_patch_v2/planar/structure_service.py`, `Ortho4XP/src/auto_patch_v2/airport/deck_signature.py`
Other lanes' (do not touch): `Ortho4XP/src/auto_patch_v2/planar/structures.py`, `Ortho4XP/src/auto_patch_v2/solve/design.py`
...
VMMC  patch    base 6fecb62b   lane v2vmmcshore      2026-09-15T09:41:56  /tmp/harness/v2vmmcshoreBASE.osm  — BASE CONTROL for lane v2vmmcshore (VMMC --tag v2vmmcshoreBASE): rc 0, 21.6 s, body_sha f328f77e1639, artifact ledger b3a8d4c01325, shared repo UNCHANGED. 11 tunnel_ramp / 19 structure_rim; census LAW-TRUE 366 ADJUDICATED 225 (airside 212) FAIL; strip_seam_tear 61 (worst 6.110 m 238%), adjacent_ground_step 6, transverse 11, airside_no_step 78, hairline_pair 68; water_pins wet 193; pav5 four faces 3.58..6.16
VMMC  patch    base 6452fd46   lane v2vmmcshore      2026-09-15T09:41:56  /tmp/harness/v2vmmcA5.osm  — LANE ARM (VMMC --tag v2vmmcA5 --no-ledger; tree one merge before the final): rc 0, 18.8 s, body_sha 9c240e89bccc, shared repo UNCHANGED. tunnels/ramps/rims 0 (bores no service 12); census LAW-TRUE 133 ADJUDICATED 55; strip_seam_tear 0, adjacent_ground_step 0, transverse 0, strip_transverse 0, airside_no_step 32, hairline_pair 76 (all out of scope); water_pins wet 0; ZERO patch nodes below 0.5 m; pav5 min 5.09. The closing build on the FINAL tree REFUSES: +22+113_big_roads.osm.bz2 is schema-stale (RULINGS 2026-09-15u), needs the owner's --refresh-data osm_layers
VMMC  patch    base 3a7747e6   lane v2vmmcshore      2026-09-15T10:22:07  /private/tmp/claude-501/-Users-noah-XPTerrainBuilder/3fc455a9-745d-4126-a4c3-38d52975e33b/scratchpad/vs/r4emit/VMMC_auto.patch.osm  — ROUND 2 ARM (replay-emitted; the harness build REFUSES on the schema-stale +22+113_big_roads layer). Matched BASE = scratchpad/vs/baseemit at 079197eb. owner probe covered by nothing; ramps 11->8, rims 19->13; ramp+rim on the sea 1543.8 -> 1.7 m2; nodes<=0.5 m 284 -> 0; strip_seam_tear 63->0, adjacent_ground_step 6->0, ramp_in_strip 18->0, transverse 11->0; LAW-TRUE 456->165 ADJUDICATED 245->55; pav5 5.09..6.12; shore trim 98,575 m2 / 6,567 m / 19 quays. The surviving stub is tunnel:-2488@0, three faces 38.8 m clear of pav5
